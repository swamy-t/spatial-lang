...
trait ScalaGenUnrolled extends ScalaGenMemories with ScalaGenSRAM {
  ...
  def getStreamsAndFIFOs(ctrl: Exp[_]): List[Exp[_]] = {
    writtenIn(ctrl).filter { x =>
      isStreamIn(x) || isFIFO(x)
    } ++ childrenOf(ctrl).flatMap(getStreamsAndFIFOs)
  }
  ...
  private def emitUnrolledLoop(
    ...
    for (i <- iters.indices) {
      if (isForever(ctrs(i))) {
        ...
        if (inputs.nonEmpty) {
          emit(
            src"def hasItems_$lhs: Boolean = " + inputs
              .map(quote)
              .map(_ + ".nonEmpty")
              .mkString(" || "))
        } else {
          ...
          emit(
            src"def hasItems_$lhs: Boolean = { val has = ${lhs}_ctr_$i < ${lhs}_iters_$i ; ${lhs}_ctr_$i += 1; has }")
        }

        open(src"while(hasItems_$lhs) {")
        iters(i).zipWithIndex.foreach {
          case (iter, j) =>
            emit(src"val $iter = Number(1,true,FixedPoint(true,32,0))")
        }
        valids(i).zipWithIndex.foreach {
          case (valid, j) => emit(src"val $valid = Bit(true,true)")
        }
      } else {
        open(src"$cchain($i).foreach{case (is,vs) => ")
        iters(i).zipWithIndex.foreach {
          case (iter, j) => emit(src"val $iter = is($j)")
        }
        valids(i).zipWithIndex.foreach {
          case (valid, j) => emit(src"val $valid = vs($j)")
        }
      }
    }
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case op @ ParSRAMLoad(sram, inds, ens) =>
      ...
      inds.indices.foreach { i =>
        open(src"val a$i = {")
        oobApply(op.mT, sram, lhs, inds(i)) {
          emit(
            src"""if (${ens(i)}) $sram.apply(${flattenAddress(dims, inds(i))}) else ${invalid(
              op.mT)}""")
        }
        close("}")
      }
      emit(
        src"Array[${op.mT}](" + inds.indices
          .map { i =>
            src"a$i"
          }
          .mkString(", ") + ")")
    ...
    case op @ ParSRAMStore(sram, inds, data, ens) =>
      ...
      open(src"val $lhs = {")
      inds.indices.foreach { i =>
        oobUpdate(op.mT, sram, lhs, inds(i)) {
          emit(
            src"if (${ens(i)}) $sram.update(${flattenAddress(dims, inds(i))}, ${data(i)})")
        }
      }
      close("}")
    ...
    case op @ ParFIFODeq(fifo, ens) =>
      ...
      ens.zipWithIndex.foreach {
        case (en, i) =>
          emit(
            src"val a$i = if ($en && $fifo.nonEmpty) $fifo.dequeue() else ${invalid(op.mT)}")
      }
      emit(
        src"Array[${op.mT}](" + ens.indices
          .map { i =>
            src"a$i"
          }
          .mkString(", ") + ")")
    ...
    case op @ ParStreamRead(strm, ens) =>
      ...
      ens.zipWithIndex.foreach {
        case (en, i) =>
          emit(
            src"val a$i = if ($en && $strm.nonEmpty) $strm.dequeue() else ${invalid(op.mT)}")
      }
      emit(
        src"Array[${op.mT}](" + ens.indices
          .map { i =>
    ...
    case op @ ParLineBufferEnq(buffer, data, ens) =>
      ...
      ens.zipWithIndex.foreach {
        case (en, i) =>
          oobUpdate(op.mT, buffer, lhs, Nil) {
            emit(src"if ($en) $buffer.enq(${data(i)})")
          }
      }
      close("}")
    ...
    case op @ ParLineBufferLoad(buffer, rows, cols, ens) =>
      ...
      ens.zipWithIndex.foreach {
        case (en, i) =>
          open(src"val a$i = {")
          oobApply(op.mT, buffer, lhs, List(rows(i), cols(i))) {
            emit(
              src"if ($en) $buffer.apply(${rows(i)},${cols(i)}) else ${invalid(op.mT)}")
          }
          close("}")
      }
      emit(
    ...
    case op @ ParRegFileStore(rf, inds, data, ens) =>
      ...
      ens.zipWithIndex.foreach {
        case (en, i) =>
          oobUpdate(op.mT, rf, lhs, inds(i)) {
            emit(
              src"if ($en) $rf.update(${flattenAddress(dims, inds(i), None)}, ${data(i)})")
          }
      }
      close("}")
    ...
    case op @ ParRegFileLoad(rf, inds, ens) =>
      ...
      ens.zipWithIndex.foreach {
        case (en, i) =>
          open(src"val a$i = {")
          oobApply(op.mT, rf, lhs, inds(i)) {
            emit(
              src"if ($en) $rf.apply(${flattenAddress(dims, inds(i), None)}) else ${invalid(op.mT)}")
          }
          close("}")
      }
      emit(
