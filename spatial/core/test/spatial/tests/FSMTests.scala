...
object DotProductFSM extends SpatialTest {
  ...
  def main() {
    ...
    assert(result == gold,
           "Result (" + result + ") did not equal expected (" + gold + ")")
    println("PASS")
  }
}
    ...
    Accel {
      ...
      FSM[Int](i => i < 128) { i =>
        ...
        if (i < 64) {
          ...
          sum := sum + Reduce(0)(0 until 16) { i =>
            sram(i)
          } { _ + _ }
        } else {
          sram load vecB(i :: i + 16)
          product := product * Reduce(0)(0 until 16) { i =>
            sram(i)
          } { _ * _ }
        }
      } { i =>
...
class FSMTests extends FlatSpec with Matchers with Exceptions {
  ...
  "OuterSwitchFSMTest" should "compile" in {
    OuterSwitchFSMTest.main(Array.empty)
  }
}
