...
class NBufCtr() extends Module {
  ...
  val effectiveCnt = Mux(cnt + io.input.start >= io.input.max,
                         cnt + io.input.start - io.input.max,
                         cnt + io.input.start)

  val nextCntDown =
    Mux(io.input.enable, Mux(cnt === 0.U, io.input.max - 1.U, cnt - 1.U), cnt)
  val nextCntUp =
    Mux(io.input.enable, Mux(cnt + 1.U === io.input.max, 0.U, cnt + 1.U), cnt)
...
class SingleCounter(val par: Int) extends Module {
  ...
  if (par > 0) {
    ...
    (0 until par).foreach { i =>
      io.output.count(i) := count + i.U * io.input.stride
    }
    (0 until par).foreach { i =>
...
class Counter(val par: List[Int]) extends Module {
  ...
  // Create counters
  val ctrs = (0 until depth).map { i =>
    Module(new SingleCounter(par(i)))
  }
  ...
  (1 until depth).foreach { i =>
    ctrs(i).io.input.saturate := io.input.saturate & ctrs
      .take(i)
      .map { ctr =>
        ctr.io.output.saturated
      }
      .reduce { _ & _ }
  }

  // Wire up the outputs
  par.zipWithIndex.foreach {
    case (p, i) =>
