...
trait PIRCommon extends PIR {
  private def collectX[T](a: Any)(func: Any => Set[T]): Set[T] = a match {
    case cu: ComputeUnit =>
      func(cu.allStages) ++ func(cu.cchains) ++ func(cu.srams)
  ...
  def localInputs(a: Any): Set[LocalComponent] = a match {
    ...
    case mem: CUMemory =>
      localInputs(
        mem.readAddr ++ mem.writeAddr ++ mem.writeStart ++ mem.writeEnd ++ mem.vector)
    case counter: CUCounter =>
      localInputs(List(counter.start, counter.end, counter.stride))
  ...
  def globalInputs(a: Any): Set[GlobalBus] = a match {
    ...
    case mem: CUMemory =>
      globalInputs(
        mem.readAddr ++ mem.writeAddr ++ mem.writeStart ++ mem.writeEnd ++ mem.vector)
    case counter: CUCounter =>
      globalInputs(List(counter.start, counter.end, counter.stride))
  ...
  def scalarInputs(a: Any): Set[ScalarBus] = globalInputs(a).collect {
    case x: ScalarBus => x
  }
  def scalarOutputs(a: Any): Set[ScalarBus] = globalOutputs(a).collect {
    case x: ScalarBus => x
  }

  def vectorInputs(a: Any): Set[VectorBus] = globalInputs(a).collect {
    case x: VectorBus => x
  }
  def vectorOutputs(a: Any): Set[VectorBus] = globalOutputs(a).collect {
    case x: VectorBus => x
  }
  ...
  def usedCChains(a: Any): Set[CUCChain] = a match {
    ...
    case stage: Stage =>
      stage.inputMems.collect { case CounterReg(cchain, _) => cchain }.toSet
    case sram: CUMemory =>
  ...
  def copyIterators(destCU: AbstractComputeUnit,
                    srcCU: AbstractComputeUnit): Map[CUCChain, CUCChain] = {
    if (destCU != srcCU) {
      val cchainCopies = srcCU.cchains.toList.map {
        case cc @ CChainCopy(name, inst, owner) => cc -> cc
        case cc @ CChainInstance(name, ctrs) =>
          cc -> CChainCopy(name, cc, srcCU)
      ...
      srcCU.iterators.foreach {
        case (iter, CounterReg(cchain, idx)) =>
          destCU.addReg(
            iter,
            CounterReg(cchainMapping.getOrElse(cchain, cchain), idx))
      }
      srcCU.valids.foreach {
        case (iter, ValidReg(cchain, idx)) =>
          destCU.addReg(iter,
                        ValidReg(cchainMapping.getOrElse(cchain, cchain), idx))
      }
      cchainMapping
  ...
  def isReadable(x: LocalComponent): Boolean = x match {
    ...
    case _: WriteAddrWire | _: ReadAddrWire | _: FeedbackAddrReg |
        _: FeedbackDataReg =>
      false
