  ...
  def main() = {
    ...
    val gold = a1.map { i =>
      a2.map { j =>
        a3.map { k =>
            a4.map { l =>
              src(i + k * tileSize).apply(j + l * tileSize)
            }
          }
          .flatten
          .reduce { _ + _ }
...
object ScatterGather extends SpatialTest {
  ...
  def scattergather[T: Meta: Num](addrs: Array[Int],
...
class UnitTests2 extends FlatSpec with Matchers {
  ...
  "MultiplexedWriteTest" should "compile" in {
    MultiplexedWriteTest.main(Array.empty)
  }
