...
trait ScalaGenSpatialFixPt extends ScalaGenBits {
  ...
  override protected def quoteConst(c: Const[_]): String = (c.tp, c) match {
    case (FixPtType(sign, int, frac), Const(c: BigDecimal)) =>
      s"Number(BigDecimal($c),true,FixedPoint($sign,$int,$frac))"
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case StringToFixPt(x) =>
      lhs.tp match {
        case FixPtType(s, i, f) =>
          emit(src"val $lhs = Number($x, FixedPoint($s,$i,$f))")
      }
    case FixRandom(Some(max)) =>
      lhs.tp match {
        case FixPtType(s, i, f) =>
          emit(src"val $lhs = Number.random($max, FixedPoint($s,$i,$f))")
      }
    case FixRandom(None) =>
      lhs.tp match {
        case FixPtType(s, i, f) =>
          emit(src"val $lhs = Number.random(FixedPoint($s,$i,$f))")
      }
