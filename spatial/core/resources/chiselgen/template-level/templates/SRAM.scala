...
class flatW(val w: Int) extends Bundle {
  ...
  override def cloneType =
    (new flatW(w)).asInstanceOf[this.type] // See chisel3 bug 358
}
class flatR(val w: Int) extends Bundle {
  ...
  override def cloneType =
    (new flatR(w)).asInstanceOf[this.type] // See chisel3 bug 358
}
class multidimW(val N: Int, val w: Int) extends Bundle {
  ...
  override def cloneType =
    (new multidimW(N, w)).asInstanceOf[this.type] // See chisel3 bug 358
}
class multidimR(val N: Int, val w: Int) extends Bundle {
  ...
  override def cloneType =
    (new multidimR(N, w)).asInstanceOf[this.type] // See chisel3 bug 358
}

class Mem1D(val size: Int, val isFifo: Boolean, bitWidth: Int = 32)
    extends Module { // Unbanked, inner 1D mem
...
class MemND(val dims: List[Int], bitWidth: Int = 32) extends Module {
  ...
  // Address flattening
  m.io.w.addr := io.w.addr.zipWithIndex
    .map {
      case (addr, i) =>
        addr * (dims.drop(i).reduce { _ * _ } / dims(i)).U
    }
    .reduce { _ + _ }
  m.io.r.addr := io.r.addr.zipWithIndex
    .map {
      case (addr, i) =>
        addr * (dims.drop(i).reduce { _ * _ } / dims(i)).U
    }
  ...
  val rInBound =
    io.r.addr.zip(dims).map { case (addr, bound) => addr < bound.U }.reduce {
      _ & _
    }
  val wInBound =
    io.w.addr.zip(dims).map { case (addr, bound) => addr < bound.U }.reduce {
      _ & _
    }
 ...
 */
class SRAM(val logicalDims: List[Int],
           val bitWidth: Int,
           val banks: List[Int],
  ...
  def this(
      tuple: (List[Int],
              ...
              List[Int],
              BankingMode)) =
    this(tuple._1, tuple._2, tuple._3, tuple._4, tuple._5, tuple._6, tuple._7)
  // Bankmode-less
  def this(logicalDims: List[Int],
           bitWidth: Int,
           banks: List[Int],
           strides: List[Int],
           wPar: List[Int],
           rPar: List[Int]) =
    this(logicalDims, bitWidth, banks, strides, wPar, rPar, BankedMemory)
  ...
  val physicalDims = bankingMode match {
    case DiagonalMemory =>
      logicalDims.zipWithIndex.map {
        case (dim, i) =>
          if (i == N - 1) math.ceil(dim.toDouble / banks.head).toInt else dim
      }
    case BankedMemory =>
      logicalDims.zip(banks).map {
        case (dim, b) => math.ceil(dim.toDouble / b).toInt
      }
  }
  ...
  val wConversions = io.w.map { wbundle =>
    ...
    val physicalAddrs = bankingMode match {
      case DiagonalMemory =>
        wbundle.addr.zipWithIndex.map {
          case (logical, i) =>
            if (i == N - 1) logical / banks.head.U else logical
        }
      case BankedMemory =>
        wbundle.addr.zip(banks).map { case (logical, b) => logical / b.U }
    }
    physicalAddrs.zipWithIndex.foreach {
      case (calculatedAddr, i) => convertedW.addr(i) := calculatedAddr
    }
    ...
    val flatBankId = bankingMode match {
      ...
      case BankedMemory =>
        val bankCoords =
          wbundle.addr.zip(banks).map { case (logical, b) => logical % b.U }
        bankCoords.zipWithIndex
          .map {
            case (c, i) => c * (banks.drop(i).reduce { _ * _ } / banks(i)).U
          }
          .reduce { _ + _ }
    }
  ...
  val rConversions = io.r.map { rbundle =>
    ...
    val physicalAddrs = bankingMode match {
      case DiagonalMemory =>
        rbundle.addr.zipWithIndex.map {
          case (logical, i) =>
            if (i == N - 1) logical / banks.head.U else logical
        }
      case BankedMemory =>
        rbundle.addr.zip(banks).map { case (logical, b) => logical / b.U }
    }
    physicalAddrs.zipWithIndex.foreach {
      case (calculatedAddr, i) => convertedR.addr(i) := calculatedAddr
    }
  ...
  m.zipWithIndex.foreach {
    case (mem, i) =>
      val bundleSelect = bankIdW.zip(convertedWVec).map {
        case (bid, wvec) => bid === i.U & wvec.en
      }
  ...
  m.zipWithIndex.foreach {
    case (mem, i) =>
      val bundleSelect = bankIdR.zip(convertedRVec).map {
        case (bid, rvec) => (bid === i.U) & rvec.en
      }
  ...
  io.output.data.zip(bankIdR).foreach {
    case (wire, id) =>
      val sel = (0 until numMems).map { i =>
        (id === i.U)
      }
  ...
  def connectWPort(wBundle: Vec[multidimW], ports: List[Int]) {
    ...
    val potentialFits =
      wPar.zipWithIndex.filter(_._1 == wBundle.length).map(_._2)
    val wId = potentialFits(
      potentialFits.map(wInUse(_)).indexWhere(_ == false))
  ...
  val wInBound = io.w
    .map { v =>
      v.addr
        .zip(logicalDims)
        .map { case (addr, bound) => addr < bound.U }
        .reduce { _ & _ }
    }
    .reduce { _ & _ }
  val rInBound = io.r
    ...
    .reduce { _ & _ }
  val writeOn = io.w.map { v =>
    v.en
  }
  val readOn = io.r.map { v =>
    v.en
  }
  val rwOn = writeOn.zip(readOn).map { case (a, b) => a & b }.reduce { _ | _ }
  val rCollide = bankIdR
    .zip(readOn)
    .map {
      case (id1, en1) =>
        bankIdR
          .zip(readOn)
          .map { case (id2, en2) => Mux((id1 === id2) & en1 & en2, 1.U, 0.U) }
          .reduce { _ + _ }
    }
    .reduce { _ + _ } != readOn.map { Mux(_, 1.U, 0.U) }.reduce { _ + _ }
  val wCollide = bankIdW
    .zip(writeOn)
    .map {
      case (id1, en1) =>
        bankIdW
    ...
    .reduce { _ + _ } != writeOn.map { Mux(_, 1.U, 0.U) }.reduce { _ + _ }
  io.debug.invalidWAddr := ~wInBound
  io.debug.invalidRAddr := ~rInBound
  io.debug.rwOn := rwOn
...
class NBufSRAM(val logicalDims: List[Int],
               val numBufs: Int,
               val bitWidth: Int, /*TODO: width, get rid of this!*/
               val banks: List[Int],
               val strides: List[Int],
               val wPar: List[Int],
               val rPar: List[Int],
               val rBundling: List[Int],
               val bPar: Int,
               val bankingMode: BankingMode)
    extends Module {
      ...
      (0 until maxR).foreach { lane => // Technically only need per read and not per buf but oh well
        ...
        val buffet = (0 until numBufs).map { p =>
          ...
          val base = if (p > 0) {
            (0 until p)
              .map { q =>
                rHashmap.getOrElse(q, List((0, 0))).map { _._1 }.reduce {
                  _ + _
                }
              }
              .reduce { _ + _ }
  ...
  (0 until numBufs).foreach { i =>
    ...
    (0 until maxR).foreach { j =>
      io.output.data(i * maxR + j) := chisel3.util.Mux1H(sel, srams.map { f =>
        f.io.output.data(j)
      })
    }
  }
