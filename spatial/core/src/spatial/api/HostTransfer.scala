...
trait HostTransferApi extends HostTransferExp { this: SpatialApi =>
  ...
  @api
  def setArg[A, T: Bits](reg: Reg[T], value: A)(
      implicit lift: Lift[A, T]): Void = {
  ...
  @api def setMem[T: Meta: Bits](dram: DRAM[T], data: Array[T]): Void =
    Void(set_mem(dram.s, data.s))
  ...
  @api def setMem[T: Meta: Bits](dram: DRAM[T], matrix: Matrix[T]): Void =
    setMem(dram, matrix.data)
  @api
  def getMatrix[T: Meta: Bits](dram: DRAM2[T])(
      implicit ctx: SrcCtx): Matrix[T] = {
...
trait HostTransferExp { this: SpatialExp =>
  ...
  /** IR Nodes **/
  case class SetArg[T: Type: Bits](reg: Exp[Reg[T]], value: Exp[T])
      extends Op[Void] {
  ...
  case class SetMem[T: Type: Bits](dram: Exp[DRAM[T]], data: Exp[MetaArray[T]])
      extends Op[Void] {
    def mirror(f: Tx)    = set_mem(f(dram), f(data))
    override def aliases = Nil
    val mT               = typ[T]
  }
  case class GetMem[T: Type: Bits](dram: Exp[DRAM[T]],
  ...
  /** Smart Constructors **/
  @internal
  def set_arg[T: Type: Bits](reg: Exp[Reg[T]], value: Exp[T]): Exp[Void] = {
  ...
  @internal
  def set_mem[T: Type: Bits](dram: Exp[DRAM[T]],
                             data: Exp[MetaArray[T]]): Exp[Void] = {
    stageWrite(dram)(SetMem(dram, data))(ctx)
  }
  @internal
  def get_mem[T: Type: Bits](dram: Exp[DRAM[T]],
                             array: Exp[MetaArray[T]]): Exp[Void] = {
