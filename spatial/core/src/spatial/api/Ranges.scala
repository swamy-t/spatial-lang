...
trait RangeApi extends RangeExp with RangeLowPriorityImplicits {
  ...
  implicit class IndexRangeOps(x: Index) {
    private def lft(x: scala.Int)(implicit ctx: SrcCtx) =
      lift[scala.Int, Index](x)
    @api def by(step: scala.Int): Range =
      range_alloc(None, x, Some(lft(step)), None)
    @api def par(p: scala.Int): Range =
      range_alloc(None, x, None, Some(lft(p)))
    @api def until(end: scala.Int): Range =
      range_alloc(Some(x), lft(end), None, None)
  ...
  implicit class intWrapper(x: scala.Int) {
    ...
    @api def until(end: Index): Range =
      range_alloc(Some(lft(x)), end, None, None)
    @api def by(step: Index): Range =
      range_alloc(None, lft(x), Some(step), None)
    @api def par(p: Index): Range = range_alloc(None, lft(x), None, Some(p))

    @api def until(end: scala.Int): Range =
      range_alloc(Some(lft(x)), lft(end), None, None)
    @api def by(step: scala.Int): Range =
      range_alloc(None, lft(x), Some(lft(step)), None)
    @api def par(p: scala.Int): Range =
      range_alloc(None, lft(x), None, Some(lft(p)))

    @api def ::(start: Index): Range =
      range_alloc(Some(start), lft(x), None, None)
    @api def ::(start: scala.Int): Range =
      range_alloc(Some(lft(start)), lft(x), None, None)

    @api def to[B: Meta](implicit cast: Cast[scala.Int, B]): B = cast(x)
  }

  // Implicitly get value of register to use in counter definitions
  implicit def regToIndexRange(x: Reg[Index])(
      implicit ctx: SrcCtx): IndexRangeOps = IndexRangeOps(x.value)
}
...
trait RangeExp { this: SpatialExp =>
  ...
  case class Range(start: Option[Index],
                   end: Index,
                   step: Option[Index],
                   p: Option[Index],
                   isUnit: Boolean) {
    @api def by(step: Index): Range =
      Range(start, end, Some(step), p, isUnit = false)
    @api def par(p: Index): Range =
      Range(start, end, step, Some(p), isUnit = false)

    @api def ::(start2: Index): Range =
      Range(Some(start2), end, start, p, isUnit = false)
  ...
  protected def range_alloc(start: Option[Index],
                            end: Index,
                            stride: Option[Index],
                            par: Option[Index],
  ...
  protected def index_to_range(x: Index)(implicit ctx: SrcCtx) =
    range_alloc(Some(x), x + 1, None, None, isUnit = true)
  ...
  case class RangeForeach(
    ...
    override def freqs =
      normal(start) ++ normal(end) ++ normal(step) ++ hot(func)
  ...
  /** Constructors **/
  @internal
  def range_foreach(start: Exp[Index],
                    end: Exp[Index],
                    step: Exp[Index],
                    func: => Exp[Void],
                    i: Bound[Index]) = {
