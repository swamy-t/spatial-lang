...
trait BitOpsExp { this: SpatialExp =>
  ...
  @internal
  def dataAsBitVector[A: Meta: Bits](x: A)(implicit ctx: SrcCtx): BitVector =
    typ[A] match {
      case tp: StructType[_] =>
        val fieldTypes = tp.fields.map { case (name, mT) => mT }
        val fields     = tp.ev(x).fields.map(_._2)
        val fieldBits = fields.zip(fieldTypes).map {
          case (field, mT @ Bits(bT)) =>
      ...
      case tp: VectorType[_] =>
        ...
        if (tp.child == BoolType) {
          ...
          val mT       = tp.child
          val Bits(bT) = mT
          val elems = List.tabulate(tp.width) { i =>
            dataAsBitVector(vector.apply(i))(mtyp(mT), mbits(bT), ctx).s
          }
  ...
  @internal
  def bitVectorSliceOrElseZero[A: Meta: Bits](x: BitVector, offset: Int)(
    ...
    if (offset < vecSize && length + offset - 1 < vecSize) {
      val fieldBits = fvT.wrapped(
        vector_slice(x.s, length + offset - 1, offset)(typ[Bool],
                                                       bits[Bool],
                                                       ctx,
                                                       fvT))
      ...
      val trueBits =
        vector_slice(x.s, max, offset)(typ[Bool], bits[Bool], ctx, fvT)
      val zeroBits = fvB2.zero.s
      // Note that this is big-endian concatenation (everything internally is big-endian)
      val fieldBits = fvT.wrapped(vector_concat[Bool, VectorN](
        Seq(trueBits, zeroBits))(typ[Bool], bits[Bool], ctx, fvT))
  ...
  @internal
  def bitVectorAsData[B: Meta: Bits](x: BitVector, enWarn: Boolean)(
      implicit vT: Meta[BitVector],
      ctx: SrcCtx): B = {
    val Bits(bT) = vT
    ...
    typ[B] match {
      case tp: StructType[_] =>
        ...
        val offsets = List.tabulate(sizes.length) { i =>
          sizes.drop(i + 1).sum
        }

        val fields = (fieldTypes, offsets).zipped.map {
          case (mT @ Bits(bT), offset) =>
      ...
      case tp: VectorType[_] =>
        ...
        if (tp.child == BoolType) {
          ...
          tp.wrapped(
            vector_new(elems)(mtyp(mT), mbits(bT), ctx, mtyp(tp))
              .asInstanceOf[Exp[B]])
        }
  ...
  @internal
  def checkLengthMismatch[A: Meta: Bits, B: Meta: Bits]()(
    ...
    if (lenA != lenB)
      warn(
        ctx,
        u"Bit length mismatch in conversion between ${typ[A]} and ${typ[B]}.")
  ...
  case class BitsAsData[T: Type: Bits](a: Exp[BitVector], mT: Type[T])
      extends Op[T] {
    def mirror(f: Tx) = bits_as_data[T](f(a))
  }

  case class DataAsBits[T: Type: Bits](a: Exp[T])(implicit mV: Type[BitVector])
      extends Op[BitVector] {
  ...
  /** Constructors **/
  @internal
  def data_as_bits[A: Type: Bits](a: Exp[A])(
      implicit mV: Type[BitVector]): Exp[BitVector] =
    stage(DataAsBits[A](a))(ctx)
  @internal def bits_as_data[A: Type: Bits](a: Exp[BitVector]): Exp[A] =
    stage(BitsAsData[A](a, typ[A]))(ctx)
