...
object Kmeans extends SpatialApp { // Regression (Dense) // Args: 8 96
  ...
  @virtualize
  def kmeans[T: Type: Num](points_in: Array[T],
                           cent_inits: Array[T],
                           numPoints: Int,
                           numCents: Int,
                           numDims: Int,
    ...
    Accel {
      Sequential { // TODO: Remove this Sequential wrapper once David fixes Accel analysis
        ...
        Sequential.Foreach(iters by 1) { epoch =>
          ...
          Foreach(K by 1 par PX) { ct =>
            centCount(ct) = max(newCents(ct, DM1), 1.to[T])
          } // Until diagonal banking is allowed
...
//          val updateMux = mux(centCount(ct) == 0.to[T], 0.to[T], newCents(ct,d) / centCount(ct))
            cts(ct, d) = mux(centCount(ct) == 0.to[T],
                             0.to[T],
                             newCents(ct, d) / centCount(ct)) //updateMux
          }
        }
  ...
  def main() {
    ...
    val pts = Array.tabulate(N) { i =>
      Array.tabulate(D) { d =>
        if (d == D - 1) 1.to[X] else random[X](element_max) + i
      }
    }
    val cnts = Array.tabulate(K) { i =>
      Array.tabulate(D) { d =>
        if (d == D - 1) 1.to[X] else random[X](element_max) + (i * N / K)
      }
    }
    ...
    for (k <- 0 until K) {
      cts(k) = Array.tabulate(D) { i =>
        cnts(k).apply(i)
      }
    }
    val ii = Array.tabulate(K) { i =>
      i
    }

    for (epoch <- 0 until iters) {
      def dist[T: Type: Num](p1: Array[T], p2: Array[T]) =
        p1.zip(p2) { (a, b) =>
            (a - b) ** 2
          }
          .reduce(_ + _)
    ...
    val cksum = result
      .zip(gold) { case (o, g) => (g < (o + margin)) && g > (o - margin) }
      .reduce { _ && _ }
