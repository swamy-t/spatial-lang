...
trait MemoryAnalyzer extends CompilerPass {
  ...
  def mergeBanking(mem: Exp[_], a: Banking, b: Banking): Banking =
    (a, b) match {
      case (StridedBanking(s1, p), StridedBanking(s2, q)) if s1 == s2 =>
        StridedBanking(s1, lcm(p, q))
      case (NoBanking, _) => b
      case (_, NoBanking) => a
      case _ =>
        warn(
          mem.ctx,
          u"${mem.tp}, defined here, appears to be addressed with mismatched strides")
        warn(mem.ctx)
        NoBanking
    }
  ...
  def mergeChannels(mem: Exp[_], a: Channels, b: Channels): Channels = {
    ...
    val memC = if (memA.nDims != memB.nDims) {
      ...
      BankedMemory(List.fill(memA.nDims)(NoBanking),
                   Math.max(memA.depth, memB.depth),
                   memA.isAccum || memB.isAccum)
    } else
      (memA, memB) match {
        case (DiagonalMemory(s1, p, d1, a1), DiagonalMemory(s2, q, d2, a2)) =>
          if (s1.zip(s2).forall { case (x, y) => x == y }) {
            ...
            warn(
              ...
              u"${mem.tp}, defined here, appears to be addressed with mismatched strides")
            warn(mem.ctx)
            BankedMemory(s1.map { _ =>
              NoBanking
            }, Math.max(d1, d2), memA.isAccum || memB.isAccum)
          }

        case (BankedMemory(b1, d1, a1), BankedMemory(b2, d2, a2)) =>
          (b1, b2) match {
            case (List(Banking(1), StridedBanking(s1, p)),
                  List(StridedBanking(s2, q), Banking(1))) if p > 1 && q > 1 =>
              DiagonalMemory(List(s2, s1),
                             lcm(p, q),
                             Math.max(d1, d2),
                             a1 || a2)
            case (List(StridedBanking(s1, p), Banking(1)),
                  List(Banking(1), StridedBanking(s2, q))) if p > 1 && q > 1 =>
              DiagonalMemory(List(s1, s2),
                             lcm(p, q),
                             Math.max(d1, d2),
                             a1 || a2)
            case _ =>
              BankedMemory(b1.zip(b2).map {
                case (x, y) => mergeBanking(mem, x, y)
              }, Math.max(d1, d2), a1 || a2)
          }
        case (DiagonalMemory(strides, p, d1, a1), BankedMemory(s2, d2, a2)) =>
          ...
          BankedMemory(
            s2.zip(a).map { case (x, y) => mergeBanking(mem, x, y) },
            Math.max(d1, d2),
            a1 || a2)
        ...
        case (BankedMemory(s1, d1, a1), DiagonalMemory(strides, p, d2, a2)) =>
          ...
          BankedMemory(
            s1.zip(a).map { case (x, y) => mergeBanking(mem, x, y) },
            Math.max(d1, d2),
            a1 || a2)
      }
  ...
  class InstanceGroup(
    ...
    lazy val revPorts
      : Array[Set[Access]] = invertPorts(accesses, ports) // Set of accesses connected for each port
    def port(x: Int): Set[Access] =
      if (x >= revPorts.length) Set.empty else revPorts(x)

    def depth =
      if (ports.values.isEmpty) 1 else ports.values.map(_.max).max + 1
    // Assumes a fixed size, dual ported memory which is duplicated, both to meet duplicates and banking factors
    def normalizedCost =
      depth * channels.duplicates * channels.memory.totalBanks
  ...
  def printGroup(group: InstanceGroup): Unit = {
    ...
    dbg(c"  Buffer Ports: ")
    group.revPorts.zipWithIndex.foreach {
      case (portAccesses, port) =>
        dbg(c"    $port: " + portAccesses.mkString(", "))
    }
  }

  def invertPorts(accesses: Iterable[Access],
                  ports: Map[Access, Set[Int]]): Array[Set[Access]] = {
    val depth =
      if (ports.values.isEmpty) 1 else ports.values.map(_.max).max + 1
    Array.tabulate(depth) { port =>
      accesses.filter { a =>
        ports(a).contains(port)
      }.toSet
    }
  }
  ...
  def bankAccessGroup(
    ...
    val group = {
      if (accesses.isEmpty)
        new InstanceGroup(None,
                          Nil,
                          (BankedMemory(Nil, 1, false), 1),
                          Map.empty,
                          Map.empty)
      else {
        val bankings = accesses.map { a =>
          bankAccess(mem, a.node)
        }
        val channels = bankings.reduce { (a, b) =>
          mergeChannels(mem, a, b)
        }
        ...
        if (writers.isEmpty && reader.isDefined) {
          ...
          val isAccum = reader.exists { read =>
            writers.exists(_.node.dependsOn(read.node))
          } || (mem match {
            case s: Dyn[_] =>
              s.dependents.exists {
                case Def(e: OpReduce[_]) =>
                  e.accum == s && reader.exists { read =>
                    writers.exists(_.ctrl == read.ctrl)
                  }
          ...
          val bufferedMemory = channels.memory match {
            case BankedMemory(banks, _, _) =>
              BankedMemory(banks, depth, isAccum)
            case DiagonalMemory(strides, banks, _, _) =>
              DiagonalMemory(strides, banks, depth, isAccum)
          }
          ...
          metapipe match {
            ...
            case Some(parent) =>
              val (nbuf, tmux) = accesses.partition { access =>
                lca(access.ctrl, parent).get == parent
              }
              ...
              def allPorts =
                List
                  ...
                  .toSet
              val bufPorts = Map(nbuf.map { a =>
                a -> Set(ports(a))
              } ++ tmux.map { a =>
                a -> allPorts
              }: _*)
              val bufSwaps = Map(nbuf.map { a =>
                a -> childContaining(parent, a)
              }: _*)
              new InstanceGroup(metapipe,
                                accesses,
                                bufferedChannels,
                                bufPorts,
                                bufSwaps)
  ...
  def reachingWrites(mem: Exp[_], reader: Access) =
    writersOf(mem) // TODO: Account for "killing" writes, write ordering

  // TODO: Other cases for coalescing?
  def coalesceMemories(mem: Exp[_],
                       instances: List[InstanceGroup]): List[InstanceGroup] = {
    val writers = writersOf(mem).toSet
    val readers = readersOf(mem).toSet

    def canMerge(grps: Set[InstanceGroup]): (Boolean, Int, Option[Ctrl]) =
      if (grps.size > 1) {
    ...
    def merge(groups: Set[InstanceGroup]): InstanceGroup =
      if (groups.size > 1) {
        ...
        new InstanceGroup(
          parent,
          accesses,
          channels = groups.map(_.channels).reduce { (a, b) =>
            mergeChannels(mem, a, b)
          },
          ports,
          groups.map(_.swaps).reduce { (a, b) =>
            a ++ b
          }
        )
      } else {
    ...
    def greedyBufferMerge(instances: Set[InstanceGroup]): Set[InstanceGroup] = {
      ...
      while (instsIn.nonEmpty) {
        ...
        val exclusive = instsIn.filter { g =>
          !isLegalPartition(Set(grp, g))
        } + grp
      ...
      while (exclusiveGrps.exists(_.nonEmpty)) {
        ...
        val mergeGrp = exclusiveGrps.flatMap { g =>
          g.find(
            _.channels.memory.costBasisBanks
              .zip(grp.channels.memory.costBasisBanks)
              .forall { case (a, b) => a % b == 0 || b % a == 0 })
        }
    ...
    def exhaustiveBufferMerge(
        instances: Set[InstanceGroup]): Set[InstanceGroup] = {
      ...
      def partitions[T](elements: Set[T], canUse: Set[T] => Boolean = {
        x: Set[T] =>
          true
      }): Iterator[Set[Set[T]]] = {
        val N = elements.size

        def getPartitions(elems: Set[T],
                          max: Int,
                          depth: Int = 0): Iterator[Set[Set[T]]] = {
          if (elems.isEmpty) Set(Set.empty[Set[T]]).iterator
            ...
            Iterator.range(1, max + 1).flatMap { c: Int =>
              ...
              subs.filter(canUse).flatMap { part: Set[T] =>
                getPartitions(elems -- part, c, depth + 1).map {
                  more: Set[Set[T]] =>
                    more + part
                }
              }
  ...
  def bank(mem: Exp[_],
    ...
    duplicatesOf(mem) = coalescedGroups.flatMap { grp =>
      grp.channels.toList
    }
  }
  ...
  override protected def process[S: Type](block: Block[S]): Block[S] = {
    ...
    localMems.foreach { mem =>
      mem.tp match {
        ...
        case _: LineBufferType[_] =>
          bank(mem, bankLineBufferAccess, LineBufferSettings)
  ...
  def indexPatternsToBanking(patterns: Seq[IndexPattern],
                             strides: Seq[Int]): Seq[Banking] = {
    ...
    val banking = (patterns, strides).zipped.map {
      case (pattern, stride) =>
        pattern match {
          case AffineAccess(Exact(a), i, b) =>
            StridedBanking(a.toInt * stride, bankFactor(i))
          case StridedAccess(Exact(a), i) =>
            StridedBanking(a.toInt * stride, bankFactor(i))
          case OffsetAccess(i, b) => StridedBanking(stride, bankFactor(i))
          case LinearAccess(i)    => StridedBanking(stride, bankFactor(i))
          case InvariantAccess(b) =>
            NoBanking // Single "bank" in this dimension
  ...
  def bankSRAMAccess(mem: Exp[_], access: Exp[_]): Channels = {
    ...
    val strides =
      if (patterns.length == 1) List(allStrides.last) else allStrides
  ...
  def bankFIFOAccess(mem: Exp[_], access: Exp[_]): Channels = {
    ...
    val innerLoopParOnly = factors.drop(1).forall { x =>
      x.isEmpty || x.forall { case Exact(c) => c == 1; case _ => false }
    }
    if (!innerLoopParOnly) {
      error(
        access.ctx,
        u"Access to memory $mem has outer loop parallelization relative to the memory definition")
      error(
        "Concurrent readers and writers of the same memory are disallowed for FIFOs.")
      error(access.ctx)
    }

    val channels = factors.flatten.map { case Exact(c) => c.toInt }.product
    (BankedMemory(Seq(StridedBanking(1, channels)),
                  depth = 1,
                  isAccum = false),
     1)
  }
  ...
  def bankLineBufferAccess(mem: Exp[_], access: Exp[_]): Channels = {
    ...
    val dims: Seq[Int] = stagedDimsOf(mem.asInstanceOf[Exp[SRAM[_]]]).map {
      case Exact(c) => c.toInt
    }
    val strides = constDimsToStrides(dims)

    val banking = access match {
      case Def(LineBufferColSlice(_, row, col, Exact(len))) =>
        Seq(NoBanking, StridedBanking(strides(1), len.toInt))
      case Def(LineBufferRowSlice(_, row, Exact(len), col)) =>
        Seq(StridedBanking(strides(0), len.toInt), NoBanking)
      case Def(LineBufferEnq(_, _, _)) =>
        Seq(NoBanking, StridedBanking(strides(1), channels))
  ...
  def bankRegFileAccess(mem: Exp[_], access: Exp[_]): Channels = {
    ...
    def bankFactor(i: Exp[Index]) = parFactorOf(i) match {
      case Exact(c) => c.toInt
    }
  ...
  def bankStream(mem: Exp[_]): Unit = {
    ...
    duplicatesOf(mem) = List(
      BankedMemory(Seq(StridedBanking(1, par)), 1, isAccum = false))
  }
}
