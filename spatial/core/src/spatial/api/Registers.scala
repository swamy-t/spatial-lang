...
trait RegApi extends RegExp { this: SpatialApi =>

  @api def ArgIn[T: Meta: Bits]: Reg[T] = Reg(argin_alloc[T](unwrap(zero[T])))
  @api def ArgOut[T: Meta: Bits]: Reg[T] =
    Reg(argout_alloc[T](unwrap(zero[T])))
  @api def HostIO[T: Meta: Bits]: Reg[T] =
    Reg(hostio_alloc[T](unwrap(zero[T])))

  @api def Reg[T: Meta: Bits]: Reg[T] = Reg(reg_alloc[T](unwrap(zero[T])))
  @api def Reg[T: Meta: Bits](reset: T): Reg[T] =
    Reg(reg_alloc[T](unwrap(reset)))
...
trait RegExp { this: SpatialExp =>
  ...
  class RegIsMemory[T: Meta: Bits] extends Mem[T, Reg] {
    def load(mem: Reg[T], is: Seq[Index], en: Bool)(implicit ctx: SrcCtx): T =
      mem.value
    def store(mem: Reg[T], is: Seq[Index], data: T, en: Bool)(
        implicit ctx: SrcCtx): Void = {
      Void(reg_write(mem.s, data.s, en.s))
    }
    def iterators(mem: Reg[T])(implicit ctx: SrcCtx): Seq[Counter] =
      Seq(Counter(0, 1, 1, 1))
  }
  ...
  case class RegWrite[T: Type: Bits](reg: Exp[Reg[T]],
                                     data: Exp[T],
                                     en: Exp[Bool])
  ...
  /** Constructors **/
  @internal def argin_alloc[T: Type: Bits](init: Exp[T]): Sym[Reg[T]] =
    stageMutable(ArgInNew[T](init))(ctx)
  @internal def argout_alloc[T: Type: Bits](init: Exp[T]): Sym[Reg[T]] =
    stageMutable(ArgOutNew[T](init))(ctx)
  @internal def hostio_alloc[T: Type: Bits](init: Exp[T]): Sym[Reg[T]] =
    stageMutable(HostIONew[T](init))(ctx)

  private[spatial] def reg_alloc[T: Type: Bits](init: Exp[T])(
      implicit ctx: SrcCtx): Sym[Reg[T]] = {
    stageMutable(RegNew[T](init))(ctx)
  }

  private[spatial] def reg_read[T: Type: Bits](reg: Exp[Reg[T]])(
      implicit ctx: SrcCtx): Sym[T] = stageCold(RegRead(reg))(ctx)

  private[spatial] def reg_write[T: Type: Bits](
      reg: Exp[Reg[T]],
      data: Exp[T],
      en: Exp[Bool])(implicit ctx: SrcCtx): Sym[Void] = {
    stageWrite(reg)(RegWrite(reg, data, en))(ctx)
  }

  /** Internal methods **/
  private[spatial] def isArgIn(x: Exp[_]): Boolean = getDef(x).exists {
    case ArgInNew(_) => true; case _ => false
  }
  private[spatial] def isArgOut(x: Exp[_]): Boolean = getDef(x).exists {
    case ArgOutNew(_) => true; case _ => false
  }
  private[spatial] def isHostIO(x: Exp[_]): Boolean = getDef(x).exists {
    case HostIONew(_) => true; case _ => false
  }
