...
trait ScalaGenSpatialBool extends ScalaGenBits {
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case RandomBool(None) =>
      emit(
        src"val $lhs = Bit(java.util.concurrent.ThreadLocalRandom.current().nextBoolean())")
    case RandomBool(Some(max)) =>
      emit(
        src"val $lhs = Bit(java.util.concurrent.ThreadLocalRandom.current().nextBoolean() && $max)")
