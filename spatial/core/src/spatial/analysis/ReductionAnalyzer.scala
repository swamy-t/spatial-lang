...
trait ReductionAnalyzer extends SpatialTraversal {
  ...
  private def identifyReduceFunc(rFunc: Block[_], a: Exp[_], b: Exp[_]) =
    rFunc.result match {
  ...
  override protected def visit(lhs: Sym[_], rhs: Op[_]) = rhs match {
    case e: OpReduce[_] =>
      ...
      getStages(e.reduce).foreach { sym =>
        reduceType(sym) = redType
      }
      getStages(e.load).foreach { sym =>
        reduceType(sym) = redType
      }
      getStages(e.store).foreach { sym =>
        reduceType(sym) = redType
      }
    ...
    case e: OpMemReduce[_, _] =>
      ...
      getStages(e.loadAcc).foreach { sym =>
        reduceType(sym) = redType
      }
      getStages(e.loadRes).foreach { sym =>
        reduceType(sym) = redType
      }
      getStages(e.storeAcc).foreach { sym =>
        reduceType(sym) = redType
      }
      reduceType(e.accum) = redType
      reduceType(lhs) = redType
