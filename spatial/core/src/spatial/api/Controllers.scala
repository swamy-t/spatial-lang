...
trait ControllerApi extends ControllerExp { this: SpatialApi =>
  ...
  protected case class MemReduceAccum[T, C[T]](accum: C[T],
                                               style: ControlStyle,
                                               zero: Option[T],
                                               fold: scala.Boolean) {

    /** 1 dimensional memory reduction **/
    def apply(domain1D: Counter)(map: Index => C[T])(reduce: (T, T) => T)(
        implicit ctx: SrcCtx,
        mem: Mem[T, C],
        mT: Type[T],
        bT: Bits[T],
        mC: Type[C[T]]): C[T] = {
      mem_reduceND(List(domain1D), accum, { x: List[Index] =>
        map(x.head)
      }, reduce, style, zero, fold)
      accum
    }

    /** 2 dimensional memory reduction **/
    def apply(domain1: Counter, domain2: Counter)(map: (Index, Index) => C[T])(
        reduce: (T, T) => T)(implicit ctx: SrcCtx,
                             mem: Mem[T, C],
                             mT: Type[T],
                             bT: Bits[T],
                             mC: Type[C[T]]): C[T] = {
      mem_reduceND(List(domain1, domain2), accum, { x: List[Index] =>
        map(x(0), x(1))
      }, reduce, style, zero, fold)
      accum
    }
    ...
    def apply(domain1: Counter, domain2: Counter, domain3: Counter)(
        map: (Index, Index, Index) => C[T])(reduce: (T, T) => T)(
        implicit ctx: SrcCtx,
        mem: Mem[T, C],
        mT: Type[T],
        bT: Bits[T],
        mC: Type[C[T]]): C[T] = {
      mem_reduceND(List(domain1, domain2, domain3), accum, { x: List[Index] =>
        map(x(0), x(1), x(2))
      }, reduce, style, zero, fold)
      accum
    }
    ...
    def apply(
        ...
        domain3: Counter,
        domain4: Counter,
        domain5plus: Counter*)(map: List[Index] => C[T])(reduce: (T, T) => T)(
        implicit ctx: SrcCtx,
        mem: Mem[T, C],
        mT: Type[T],
        bT: Bits[T],
        mC: Type[C[T]]): C[T] = {
      mem_reduceND(List(domain1, domain2, domain3, domain4) ++ domain5plus,
                   accum,
                   map,
                   reduce,
                   style,
  ...
  protected case class MemReduceClass(style: ControlStyle) {
    def apply[T, C[T]](accum: C[T]) =
      MemReduceAccum[T, C](accum, style, None, fold = false)
    def apply[T, C[T]](accum: C[T], zero: T) =
      MemReduceAccum[T, C](accum, style, Some(zero), fold = false)
  }

  protected case class MemFoldClass(style: ControlStyle) {
    def apply[T, C[T]](accum: C[T]) =
      MemReduceAccum[T, C](accum, style, None, fold = true)
    def apply[T, C[T]](accum: C[T], zero: T) =
      MemReduceAccum[T, C](accum, style, Some(zero), fold = true)
  }

  protected class ReduceAccum[T](accum: Option[Reg[T]],
                                 style: ControlStyle,
                                 zero: Option[T],
                                 fold: Option[T]) {

    /** 1 dimensional reduction **/
    def apply(domain1D: Counter)(map: Index => T)(reduce: (T, T) => T)(
        implicit ctx: SrcCtx,
        mT: Type[T],
        bits: Bits[T]): Reg[T] = {
      val acc = accum.getOrElse(Reg[T])
      reduceND(List(domain1D), acc, { x: List[Index] =>
        map(x.head)
      }, reduce, style, zero, fold)
      acc
    }
    ...
    def apply(domain1: Counter, domain2: Counter)(map: (Index, Index) => T)(
        reduce: (T, T) => T)(implicit ctx: SrcCtx,
                             mT: Type[T],
                             bits: Bits[T]): Reg[T] = {
      val acc = accum.getOrElse(Reg[T])
      reduceND(List(domain1, domain2), acc, { x: List[Index] =>
        map(x(0), x(1))
      }, reduce, style, zero, fold)
      acc
    }
    ...
    def apply(domain1: Counter, domain2: Counter, domain3: Counter)(
        map: (Index, Index, Index) => T)(reduce: (T, T) => T)(
        implicit ctx: SrcCtx,
        mT: Type[T],
        bits: Bits[T]): Reg[T] = {
      val acc = accum.getOrElse(Reg[T])
      reduceND(List(domain1, domain2, domain3), acc, { x: List[Index] =>
        map(x(0), x(1), x(2))
      }, reduce, style, zero, fold)
      acc
    }
    ...
    def apply(
        ...
        domain3: Counter,
        domain4: Counter,
        domain5plus: Counter*)(map: List[Index] => T)(reduce: (T, T) => T)(
        implicit ctx: SrcCtx,
        mT: Type[T],
        bits: Bits[T]): Reg[T] = {
      val acc = accum.getOrElse(Reg[T])
      reduceND(List(domain1, domain2, domain3, domain4) ++ domain5plus,
               acc,
               map,
               reduce,
               style,
  ...
  protected case class ReduceClass(style: ControlStyle)
      extends ReduceAccum(None, style, None, None) {
    ...
    def apply(zero: scala.Int) =
      new ReduceAccum(Some(Reg[Int32](int2fixpt[TRUE, _32, _0](zero))),
                      style,
                      Some(lift[Int, Int32](zero)),
                      None)
    def apply(zero: scala.Long) =
      new ReduceAccum(Some(Reg[Int64](long2fixpt[TRUE, _64, _0](zero))),
                      style,
                      Some(lift[Long, Int64](zero)),
                      None)
    def apply(zero: scala.Float) =
      new ReduceAccum(Some(Reg[Float32](float2fltpt[_24, _8](zero))),
                      style,
                      Some(lift[Float, Float32](zero)),
                      None)
    def apply(zero: scala.Double) =
      new ReduceAccum(Some(Reg[Float64](double2fltpt[_53, _11](zero))),
                      style,
                      Some(lift[Double, Float64](zero)),
                      None)
    ...
    def apply[T](accum: Reg[T]) =
      new ReduceAccum(Some(accum), style, None, None)
  }
  ...
  protected class ForeachClass(style: ControlStyle) {
    ...
    def apply(domain1D: Counter)(func: Index => Void)(
        implicit ctx: SrcCtx): Void = {
      foreachND(List(domain1D), { x: List[Index] =>
        func(x.head)
      }, style)
      ()
    }
    ...
    def apply(domain1: Counter, domain2: Counter)(
        func: (Index, Index) => Void)(implicit ctx: SrcCtx): Void = {
      foreachND(List(domain1, domain2), { x: List[Index] =>
        func(x(0), x(1))
      }, style)
      ()
    }
    ...
    def apply(domain1: Counter, domain2: Counter, domain3: Counter)(
        func: (Index, Index, Index) => Void)(implicit ctx: SrcCtx): Void = {
      foreachND(List(domain1, domain2, domain3), { x: List[Index] =>
        func(x(0), x(1), x(2))
      }, style)
      ()
    }
    ...
    def apply(domain1: Counter,
              domain2: Counter,
              domain3: Counter,
              domain4: Counter,
              domain5plus: Counter*)(func: List[Index] => Void)(
        implicit ctx: SrcCtx): Void = {
      foreachND(List(domain1, domain2, domain3, domain4) ++ domain5plus,
                func,
                style)
      ()
    }
    def apply(domain: Seq[Counter])(func: List[Index] => Void)(
        implicit ctx: SrcCtx): Void = {
...
trait ControllerExp { this: SpatialExp =>
  ...
  private[spatial] def accel_blk(func: => Void, ctr: Counter)(
      implicit ctx: SrcCtx): Controller = {
    if (isForever(ctr.s)) {
      accel_blk(func, isForever = true)
    } else {
      accel_blk({
        foreachND(Seq(ctr), { _: List[Index] =>
          func
        }, SeqPipe)
        ()
      }, false)
  ...
  private[spatial] def accel_blk(func: => Void, isForever: Boolean)(
      implicit ctx: SrcCtx): Controller = {
  ...
  private[spatial] def unit_pipe(func: => Void, style: ControlStyle)(
      implicit ctx: SrcCtx): Controller = {
    val fFunc = () => unwrap(func)
  ...
  private[spatial] def parallel_pipe(func: => Void)(
      implicit ctx: SrcCtx): Controller = {
    val fFunc = () => unwrap(func)
  ...
  private[spatial] def foreachND(
    ...
    val iters = List.tabulate(domain.length) { _ =>
      fresh[Index]
    }
  ...
  private[spatial] def reduceND[T: Meta: Bits](
    ...
    val stBlk = stageColdLambda(reg.s, rBlk.result) {
      unwrap(reg := wrap(rBlk.result))
    }
    ...
    val pipe = stageEffectful(
      OpReduce[T](cchain.s, reg.s, mBlk, ldBlk, rBlk, stBlk, z, f, rV, iters),
      effects)(ctx)
    styleOf(pipe) = style
    levelOf(pipe) = InnerControl // Fixed in Level Analyzer
    Controller(pipe)
  ...
  private[spatial] def mem_reduceND[T: Meta: Bits, C[T]](
    ...
    val rV = (fresh[T], fresh[T])
    val itersMap = List.tabulate(domain.length) { _ =>
      fresh[Index]
    }

    val ctrsRed = mem.iterators(accum)
    val itersRed = ctrsRed.map { _ =>
      fresh[Index]
    }

    val mBlk = stageColdBlock { map(wrap(itersMap)).s }
    val rBlk = stageColdBlock { reduce(wrap(rV._1), wrap(rV._2)).s }
    val ldResBlk = stageColdLambda(mBlk.result) {
      mem.load(wrap(mBlk.result), wrap(itersRed), true).s
    }
    val ldAccBlk = stageColdLambda(accum.s) {
      mem.load(accum, wrap(itersRed), true).s
    }
    val stAccBlk = stageColdLambda(accum.s, rBlk.result) {
      mem.store(accum, wrap(itersRed), wrap(rBlk.result), true).s
    }
    ...
    val node = stageEffectful(OpMemReduce[T, C](cchainMap.s,
                                                cchainRed.s,
                                                accum.s,
                                                mBlk,
                                                ldResBlk,
  ...
  /** IR Nodes **/
  case class Hwblock(func: Block[Void], isForever: Boolean)
  ...
  case class UnitPipe(en: Seq[Exp[Bool]], func: Block[Void])
      extends EnabledController {
    def mirrorWithEn(f: Tx, addEn: Seq[Exp[Bool]]) =
      op_unit_pipe(f(en) ++ addEn, f(func))
  }

  case class ParallelPipe(en: Seq[Exp[Bool]], func: Block[Void])
      extends EnabledController {
    def mirrorWithEn(f: Tx, addEn: Seq[Exp[Bool]]) =
      op_parallel_pipe(f(en) ++ addEn, f(func))
  }

  case class OpForeach(cchain: Exp[CounterChain],
                       func: Block[Void],
  ...
  case class OpReduce[T: Type: Bits](
    ...
    override def inputs =
      dyns(cchain) ++ dyns(map) ++ dyns(reduce) ++ dyns(accum) ++ dyns(load) ++ dyns(
        store) ++ dyns(ident)
  ...
  case class OpMemReduce[T: Type: Bits, C[T]](
  ...
  )(implicit val mem: Mem[T, C], val mC: Type[C[T]])
      extends Op[Controller] {
    def mirror(f: Tx) =
      op_mem_reduce(f(cchainMap),
                    f(cchainRed),
                    f(accum),
                    f(map),
                    f(loadRes),
    ...
    override def inputs =
      dyns(cchainMap) ++ dyns(cchainRed) ++ dyns(accum) ++ dyns(map) ++ dyns(
        reduce) ++
        dyns(ident) ++ dyns(loadRes) ++ dyns(loadAcc) ++ dyns(storeAcc)
    override def binds =
      super.binds ++ itersMap ++ itersRed ++ List(rV._1, rV._2)
    override def aliases = Nil
  ...
  /** Constructors **/
  def op_accel(func: => Exp[Void], isForever: Boolean)(
  ...
  def op_unit_pipe(en: Seq[Exp[Bool]], func: => Exp[Void])(
      implicit ctx: SrcCtx): Sym[Controller] = {
    val fBlk    = stageColdBlock { func }
  ...
  def op_parallel_pipe(en: Seq[Exp[Bool]], func: => Exp[Void])(
      implicit ctx: SrcCtx): Sym[Controller] = {
    val fBlk    = stageColdBlock { func }
    val effects = fBlk.summary
  ...
  def op_foreach(
      ...
      func: => Exp[Void],
      iters: List[Bound[Index]])(implicit ctx: SrcCtx): Sym[Controller] = {
    val fBlk    = stageColdBlock { func }
  ...
  def op_reduce[T: Type: Bits](
    ...
    val effects = mBlk.summary andAlso ldBlk.summary andAlso rBlk.summary andAlso stBlk.summary
    stageEffectful(OpReduce[T](cchain,
                               reg,
                               mBlk,
                               ldBlk,
                               rBlk,
  ...
  def op_mem_reduce[T: Type: Bits, C[T]](
    ...
    stageEffectful(OpMemReduce[T, C](cchainMap,
                                     cchainRed,
                                     accum,
                                     mBlk,
                                     ldResBlk,
