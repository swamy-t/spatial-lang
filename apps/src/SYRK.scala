...
object SYRK_col extends SpatialApp { // Regression (Dense) // Args: 64
  ...
  def main() = {
    ...
    val C_check = C.flatten.zip(AAT.flatten) { (a, b) =>
      a + b
    } // C + A*A'

    val C_computed = Array.tabulate(N * N) { i =>
      result(i)
    }
    val cksum = C_computed
      .zip(C_check) { (a, b) =>
