...
trait PIRScheduler extends PIRTraversal {
  ...
  override protected def postprocess[S: Type](block: Block[S]): Block[S] = {
    ...
    for (cu <- mappingOut.values) {
      ...
      if (cu.srams.nonEmpty) {
        ...
        for (sram <- cu.srams) {
          dbg(
            s"""  $sram [${sram.mode}] (sym: ${sram.mem}, reader: ${sram.reader})""")
          dbg(s"""    banking   = ${sram.banking
            .map(_.toString)
            .getOrElse("N/A")}""")
          dbg(s"""    vector    = ${sram.vector
            .map(_.toString)
            .getOrElse("N/A")}""")
          dbg(s"""    writeAddr = ${sram.writeAddr
            .map(_.toString)
            .getOrElse("N/A")}""")
          dbg(s"""    readAddr  = ${sram.readAddr
            .map(_.toString)
            .getOrElse("N/A")}""")
          dbg(s"""    start     = ${sram.writeStart
            .map(_.toString)
            .getOrElse("N/A")}""")
          dbg(s"""    end       = ${sram.writeEnd
            .map(_.toString)
            .getOrElse("N/A")}""")
          dbg(s"""    swapWrite = ${sram.swapWrite
            .map(_.toString)
            .getOrElse("N/A")}""")
          dbg(s"""    swapRead  = ${sram.swapRead
            .map(_.toString)
            .getOrElse("N/A")}""")
          dbg(s"""    writeCtrl = ${sram.writeCtrl
            .map(_.toString)
            .getOrElse("N/A")}""")
        }
      }
  ...
  def schedulePCU(pipe: Symbol, pcu: PCU) {
    ...
    pcu.computeStages.foreach { stage =>
      scheduleStage(stage, ctx)
    }
  ...
  def allocateAddrReg(sram: CUMemory,
                      addr: Symbol,
                      ctx: CUContext,
                      write: Boolean,
                      local: Boolean = false): LocalComponent = {
    val wire =
      if (write && local) FeedbackAddrReg(sram)
  ...
  def writeAddrToStage(mem: Symbol,
                       addr: Symbol,
                       ctx: CUContext,
                       local: Boolean = false) {
    ...
    ctx.memories(mem).foreach { sram =>
      sram.writeAddr = Some(
        allocateAddrReg(sram, addr, ctx, write = true, local = local)
          .asInstanceOf[WriteAddr]) //TODO
    }
  }
  def fifoOnWriteToStage(mem: Symbol,
                         start: Option[Symbol],
                         end: Option[Symbol],
                         ctx: CUContext) {
  ...
  def bufferWrite(mem: Symbol,
                  data: Symbol,
                  isdim: Option[(Seq[Exp[Index]], Seq[Exp[Index]])],
                  ctx: CUContext) {
    if (isReadInPipe(mem, ctx.pipe)) {
      ...
      addrStages.foreach { stage =>
        scheduleStage(stage, ctx)
      }
      addr.foreach { a =>
        writeAddrToStage(mem, a, ctx, local = true)
      }
  ...
  def allocateFifoPop(lhs: Symbol, fifo: Symbol, ctx: CUContext) =
    writersOf(fifo).head.ctrlNode match {
      /*case tx@Def(e:BurstLoad[_]) =>
      val dram = allocateDRAM(tx, e.dram, MemLoad)
      ctx.addReg(lhs, VectorIn(PIRDRAMDataIn(dram)))*/

      case x =>
        ctx.memOption(fifo, lhs) match {
          case Some(sram) =>
  ...
  def allocateSRAMRead(lhs: Symbol,
                       mem: Symbol,
                       dim: Seq[Exp[Index]],
                       is: Seq[Exp[Index]],
                       ctx: CUContext) {
    ...
    sram.readAddr = Some(
      allocateAddrReg(sram, addr, ctx, write = false, local = true)
        .asInstanceOf[ReadAddr]) //TODO
    ctx.addReg(lhs, SRAMReadReg(sram))
  }

  def allocateFifoPush(fifo: Symbol, data: Symbol, ctx: CUContext) =
    readersOf(fifo).head.ctrlNode match {
  ...
  def allocateRegWrite(writer: Symbol,
                       reg: Symbol,
                       data: Symbol,
                       ctx: CUContext) {
  ...
  def mapNodeToStage(lhs: Symbol, rhs: Def, ctx: CUContext) = rhs match {
    ...
    case SRAMLoad(mem, dim, is, ofs, en) =>
      allocateSRAMRead(lhs, mem, dim, is, ctx)
    case ParSRAMLoad(mem, addr, ens) =>
      allocateSRAMRead(lhs, mem, stagedDimsOf(mem), addr.head, ctx)

    case ListVector(elems) => ctx.addReg(lhs, ctx.reg(elems.head))
    case VectorApply(vec, idx) =>
      if (idx != 0)
        throw new Exception(
          "Expected parallelization of 1 in inner loop in PIR gen")
      ctx.addReg(lhs, ctx.reg(vec))

    case RegRead(reg) =>
      val input = ctx.cu.getOrElse(reg) {
        allocateLocal(reg, ctx.pipe, read = Some(lhs))
      }
    ...
    case SRAMStore(mem, dims, is, ofs, data, en) =>
      bufferWrite(mem, data, Some(is, dims), ctx)
    case ParSRAMStore(sram, addr, data, ens) =>
      bufferWrite(sram, data.head, Some(addr.head, stagedDimsOf(sram)), ctx)

    case RegWrite(reg, data, en) => allocateRegWrite(lhs, reg, data, ctx)

    // --- Constants
    case c if isConstant(lhs) =>
      ctx.cu.getOrElse(lhs) { allocateLocal(lhs, ctx.pipe) }
  ...
  def opStageToStage(op: PIROp,
    ...
    if (isReduce) {
      ...
      val inputRegs = ins.map { in =>
        ctx.reg(in)
      }
      val isControlStage = inputRegs.nonEmpty && !inputRegs.exists { reg =>
        !isControl(reg)
      }
      val hasControlLogic = inputRegs.nonEmpty && inputRegs.exists { reg =>
        isControl(reg)
      }

      if (isControlStage) {
        val n = ctx.controlStageNum
        val inputs = inputRegs.map { reg =>
          ctx.refIn(reg, n)
        }
      ...
      // HACK: Skip control logic generation for now...
        ...
        val skip =
          inputRegs.drop(1).find { case _: ConstReg => false; case _ => true }
