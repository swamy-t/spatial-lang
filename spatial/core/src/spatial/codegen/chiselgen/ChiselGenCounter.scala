...
trait ChiselGenCounter extends ChiselCodegen with FileDependencies {
  ...
  def emitCounterChain(lhs: Exp[_],
                       ctrs: Seq[Exp[Counter]],
                       suffix: String = ""): Unit = {
    ...
    emit(src"""val ${lhs}${suffix}_strides = List(${counter_data
      .map(_._3)
      .mkString(",")}) // TODO: Safe to get rid of this and connect directly?""")
    emit(src"""val ${lhs}${suffix}_maxes = List(${counter_data
      .map(_._2)
      .mkString(",")}) // TODO: Safe to get rid of this and connect directly?""")
    emit(src"""val ${lhs}${suffix}_starts = List(${counter_data
      .map { q =>
        "0.U"
      }
      .mkString(",")}) """)
    emit(
      src"""val ${lhs}${suffix} = Module(new templates.Counter(List(${counter_data
        .map(_._4)
        .mkString(",")}))) // Par of 0 creates forever counter""")
    emit(
      src"""${lhs}${suffix}.io.input.maxes.zip(${lhs}${suffix}_maxes).foreach { case (port,max) => port := max.number }""")
    emit(
      src"""${lhs}${suffix}.io.input.strides.zip(${lhs}${suffix}_strides).foreach { case (port,stride) => port := stride.number }""")
    emit(
      src"""${lhs}${suffix}.io.input.starts.zip(${lhs}${suffix}_starts).foreach { case (port,start) => port := start.number }""")
    ...
    emit(
      src"""val ${lhs}${suffix}_maxed = ${lhs}${suffix}.io.output.saturated""")
    ctrs.zipWithIndex.foreach {
      case (c, i) =>
        val x = c match {
        ...
        emit(
          s"""val ${quote(c)}${suffix} = (0 until $x).map{ j => ${quote(lhs)}${suffix}.io.output.counts($i + j) }""")
    }
  ...
  private def getCtrSuffix(head: Exp[_]): String = {
    if (parentOf(head).isDefined) {
      if (styleOf(parentOf(head).get) == StreamPipe) { src"_copy${head}" } else {
        getCtrSuffix(parentOf(head).get)
      }
    } else {
      "NO_SUFFIX_ERROR"
    }
