...
case class IdRange(start: Int, end: Int) {
  ...
  def contains(x: UInt) =
    if (start + 1 == end) { UInt(start) === x } else if (isPow2(end - start) && ((end | start) & (end - start - 1)) == 0) {
...
case class TransferSizes(min: Int, max: Int) {
  ...
  def containsLg(x: UInt) =
    if (none) Bool(false)
    else if (min == max) { UInt(log2Ceil(min)) === x } else {
      UInt(log2Ceil(min)) <= x && x <= UInt(log2Ceil(max))
    }
...
object AddressSet {
  def misaligned(base: BigInt,
                 size: BigInt,
                 tail: Seq[AddressSet] = Seq()): Seq[AddressSet] = {
