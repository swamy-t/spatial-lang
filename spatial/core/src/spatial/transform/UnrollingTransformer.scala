...
trait UnrollingTransformer extends ForwardTransformer { self =>
  ...
  def strMeta(e: Exp[_]): Unit = metadata.get(e).foreach { m =>
    dbgs(c" - ${m._1}: ${m._2}")
  }
  ...
  def inReduction[T](blk: => T): T =
    duringClone { e =>
      if (SpatialConfig.enablePIR) reduceType(e) = None
    } { blk }
  ...
  def registerAccess(original: Exp[_],
                     unrolled: Exp[_] /*, pars: Option[Seq[Int]]*/ ): Unit = {
    val reads = unrolled match {
      case LocalReader(rds) => rds.map(_._1); case _ => Nil
    }
    val writes = unrolled match {
      case LocalWriter(wrts) => wrts.map(_._1); case _ => Nil
    }
    //val accesses = reads ++ writes

    val unrollInts: Seq[Int] = accessPatternOf
      .get(original)
      .map { patterns =>
        patterns.map(
          _.index
    ...
    reads.foreach { mem =>
      ...
      if (origDispatches.size != 1) {
        error(
          c"Readers should have exactly one dispatch, $original -> $unrolled had ${origDispatches.size}")
        sys.exit()
      }
      val dispatches = origDispatches.flatMap { orig =>
        ...
        val banking = duplicatesOf(mem).apply(orig) match {
          ...
          case diagonal: DiagonalMemory =>
            Seq(diagonal.banks) ++ List.fill(diagonal.nDims - 1)(1)
        }

        // Address channels taken care of by banking
        val bankedChannels = accessPatternOf
          .get(original)
          .map { patterns =>
            val iters = patterns.map(_.index)
            iters.distinct.map {
              case x @ Some(i) =>
                val requiredBanking = parFactorOf(i) match {
                  case Exact(p) => p.toInt
                }
        ...
        val dispatchStart = orig + duplicates * others.count { p =>
          p == unrollInts
        }
        pachinko += (original, mem, orig) -> (unrollInts +: others)

        val dispatches = List
          .tabulate(duplicates) { i =>
            dispatchStart + i
          }
          .toSet
        ...
        dispatches.foreach { d =>
          portsOf(unrolled, mem, d) = portsOf(unrolled, mem, orig)
        }
        dispatches
      }
  ...
  /**
    ...
    **/
  case class Unroller(cchain: Exp[CounterChain],
                      inds: Seq[Bound[Index]],
                      isInnerLoop: Boolean) {
    val fs = countersOf(cchain).map(isForever)

    // Don't unroll inner loops for CGRA generation
    val Ps = if (isInnerLoop && SpatialConfig.enablePIR) inds.map { _ =>
      1
    } else parFactorsOf(cchain).map { case Exact(c) => c.toInt }

    val P = Ps.product
    val N = Ps.length
    val prods = List.tabulate(N) { i =>
      Ps.slice(i + 1, N).product
    }
    val indices: Seq[Seq[Bound[Index]]] = Ps.map { p =>
      List.fill(p) { fresh[Index] }
    }
    val indexValids: Seq[Seq[Bound[Bool]]] = Ps.map { p =>
      List.fill(p) { fresh[Bool] }
    }

    // Valid bits corresponding to each lane
    lazy val valids: List[Seq[Exp[Bool]]] = List.tabulate(P) { p =>
      val laneIdxValids =
        indexValids.zip(parAddr(p)).map { case (vec, i) => vec(i) }
      laneIdxValids ++ validBits
    }

    def size = P

    def parAddr(p: Int) = List.tabulate(N) { d =>
      (p / prods(d)) % Ps(d)
    }
    ...
    def split[T: Type](orig: Sym[T], vec: Exp[Vector[_]])(
        implicit ctx: SrcCtx): List[Exp[T]] = map { p =>
      val element =
        vector_apply[T](vec.asInstanceOf[Exp[Vector[T]]], p)(typ[T], ctx)
    ...
    def unify(orig: Exp[_], unrolled: Exp[_]): List[Exp[_]] = {
      foreach { p =>
        register(orig -> unrolled)
      }
      List(unrolled)
    }

    // Same symbol for all lanes
    def isCommon(e: Exp[_]) =
      contexts
        .map { p =>
          f(e)
        }
  ...
  val writeParallelizer
    : PartialFunction[(Unroller, Def, SrcCtx), Exp[Void]] = {
    case (lanes, e @ RegFileStore(reg, inds, data, en), ctx) =>
      val addrs = lanes.map { p =>
        inds.map(f(_))
      }
      val datas = lanes.map { p =>
        f(data)
      }
      val ens = lanes.map { p =>
        f(en)
  ...
  val readParallelizer
    : PartialFunction[(Unroller, Def, SrcCtx), Exp[Vector[_]]] = {
    ...
    case (lanes, e @ LineBufferLoad(lb, row, col, en), ctx) =>
      val rows = lanes.map { p =>
        f(row)
      }
      val cols = lanes.map { p =>
        f(col)
      }
      val ens = lanes.map { p =>
        f(en)
  ...
  /**
    ...
    **/
  def unroll[T](lhs: Sym[T], rhs: Op[T], lanes: Unroller)(
      implicit ctx: SrcCtx): List[Exp[_]] = rhs match {
    // Account for the edge ca e with FIFO writing
    case LocalReader(reads) if reads.forall { r =>
          lanes.isCommon(r.mem)
        } && readParallelizer.isDefinedAt((lanes, rhs, ctx)) =>
      val lhs2 = readParallelizer.apply((lanes, rhs, ctx))
      transferMetadata(lhs, lhs2)
      cloneFuncs.foreach { func =>
        func(lhs2)
      }
    ...
    case LocalWriter(writes)
        if writes.forall(w => lanes.isCommon(w.mem)) && writeParallelizer
          .isDefinedAt((lanes, rhs, ctx)) =>
    ...
    case e: OpForeach =>
      unrollControllers(lhs, rhs, lanes) { unrollForeachNode(lhs, e) }
    case e: OpReduce[_] =>
      unrollControllers(lhs, rhs, lanes) { unrollReduceNode(lhs, e) }
    case e: OpMemReduce[_, _] =>
      unrollControllers(lhs, rhs, lanes) { unrollMemReduceNode(lhs, e) }
    case _ if isControlNode(lhs) =>
      unrollControllers(lhs, rhs, lanes) { cloneOp(lhs, rhs) }
  ...
  def unrollControllers[T](lhs: Sym[T], rhs: Op[T], lanes: Unroller)(
      unroll: => Exp[_]) = {
  ...
  /**
    ...
    */
  def unrollMap[T: Type](func: Block[T], lanes: Unroller)(
      implicit ctx: SrcCtx): List[Exp[T]] = {
  ...
  def unrollForeach(
    ...
    val lhs2 =
      stageEffectful(UnrolledForeach(globalValids, cchain, blk, is, vs),
                     effects.star)(ctx)
    transferMetadata(lhs, lhs2)
  ...
  def unrollReduceTree[T: Type: Bits](
    ...
    case Some(z) =>
      val validInputs =
        inputs.zip(valids).map { case (in, v) => math_mux(v, in, z) }
      reduceTree(validInputs) { (x: Exp[T], y: Exp[T]) =>
        reduce(x, y)
      }
    ...
    case None =>
      ...
      reduceTree(inputsWithValid) {
        (x: (Exp[T], Exp[Bool]), y: (Exp[T], Exp[Bool])) =>
  ...
  def unrollReduceAccumulate[T: Type: Bits](
    ...
    def reduce(x: Exp[T], y: Exp[T]) = withSubstScope(rV._1 -> x, rV._2 -> y) {
      inlineBlock(rFunc)
    }
    ...
    val result = inReduction {
      ...
      val isFirst = reduceTree(iters.map { i =>
        fix_eql(i, int32(0))
      }) { (x, y) =>
        bool_and(x, y)
      }
    ...
    inReduction {
      withSubstScope(rFunc.result -> result) { inlineBlock(store) }
    }
  }
  ...
  def unrollReduce[T](
    ...
    val blk = stageColdLambda(f(accum)) {
      ...
      if (isOuterControl(lhs)) {
        ...
        Pipe {
          Void(
            unrollReduceAccumulate[T](values,
                                      valids(),
                                      ident,
                                      fold,
                                      rFunc,
    ...
    val rFunc2 = withSubstScope(rV._1 -> rV2._1, rV._2 -> rV2._2) {
      transformBlock(rFunc)
    }

    val effects = blk.summary
    val lhs2 = stageEffectful(
      UnrolledReduce(globalValids, cchain, accum, blk, rFunc2, inds2, vs, rV2)(
        mT,
        mC),
      effects.star)(ctx)
    transferMetadata(lhs, lhs2)
  ...
  def unrollReduceNode[T](lhs: Sym[_], rhs: OpReduce[T])(
      implicit ctx: SrcCtx) = {
    val OpReduce(cchain,
                 accum,
                 map,
  ...
  def unrollMemReduce[T, C[T]](
    ...
    val blk = stageColdLambda(f(accum)) {
      ...
      if (isUnitCounterChain(cchainRed)) {
        ...
        op_unit_pipe(
          globalValids, {
            val values = inReduction {
              mems.map { mem =>
                withSubstScope(partial -> mem) { inlineBlock(loadRes)(mT) }
              }
            }
            val foldValue =
              if (fold) { Some(inlineBlock(loadAcc)(mT)) } else None
            inReduction {
              unrollReduceAccumulate[T](values,
                                        mvalids(),
                                        ident,
        ...
        val rBlk = stageColdBlock {
          ...
          reduceLanes.foreach { p =>
            ...
            val valids = mvalids().map { mvalid =>
              bool_and(mvalid, laneValid)
            }
            ...
            val result = inReduction {
              ...
              val isFirst = reduceTree(isMap2.map { is =>
                fix_eql(is.head, int32(0))
              }) { (x, y) =>
                bool_and(x, y)
              }
        ...
        val rpipe = stageEffectful(
          UnrolledForeach(Seq(bool(true)), cchainRed, rBlk, isRed2, rvs),
          effects.star)(ctx)
    ...
    val lhs2 = stageEffectful(UnrolledReduce(globalValids,
                                             cchainMap,
                                             accum,
                                             blk,
                                             rFunc2,
  ...
  def unrollMemReduceNode[T, C[T]](lhs: Sym[_], rhs: OpMemReduce[T, C])(
      implicit ctx: SrcCtx) = {
    val OpMemReduce(cchainMap,
                    cchainRed,
                    accum,
                    func,
                    loadRes,
    ...
    unrollMemReduce(lhs,
                    ...
                    rV,
                    itersMap,
                    itersRed)(rhs.mT, rhs.bT, rhs.mC, ctx)
  }

  def cloneOp[A](lhs: Sym[A], rhs: Op[A]): Exp[A] = {
    def cloneOrMirror(lhs: Sym[A], rhs: Op[A])(implicit mA: Type[A],
                                               ctx: SrcCtx): Exp[A] =
      (lhs match {
    ...
    val (lhs2, isNew) = transferMetadataIfNew(lhs) {
      cloneOrMirror(lhs, rhs)(mtyp(lhs.tp), lhs.ctx)
    }
    ...
    if (isNew) cloneFuncs.foreach { func =>
      func(lhs2)
    }
