...
class FIFOTests(c: FIFO) extends PeekPokeTester(c) {
  ...
  def push(inc: Boolean = true) {
    (0 until c.pW).foreach { i =>
      poke(c.io.in(i), pushElement + i)
    }
    poke(c.io.push, 1)
    step(1)
    poke(c.io.push, 0)
  ...
  def pop(inc: Boolean = true) {
    ...
    if (inc) {
      ...
      (0 until c.pR).foreach { i =>
        expect(c.io.out(i), popElement + i)
      }
      popElement += c.pR
    }
    step(1)
  ...
  (0 until (p / c.pR)).foreach { i =>
    pop(false)
  }
  expect(c.io.debug.overread, 1)
  (0 until (p / c.pW)).foreach { i =>
    push(false)
  }
  expect(c.io.debug.overread, 0)
  (0 until c.depth / c.pW).foreach { i =>
    push(false)
  }
  ...
  expect(c.io.debug.overwrite, 0)
  (0 until c.depth / c.pR).foreach { i =>
    pop(false)
  }
