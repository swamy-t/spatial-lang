...
trait SimGenFixPt extends SimCodegen {
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case FixConvert(x) if hw =>
      lhs.tp match {
        case FixPtType(s, i, f) =>
          delay(lhs,
                rhs,
                src"Number($x.value, $x.valid, FixedPoint($s,$i,$f))")
      }
    ...
    case FixRandom(Some(max)) if !hw =>
      lhs.tp match {
        case IntType() => emit(src"val $lhs = scala.util.Random.nextInt($max)")
        case LongType() =>
          emit(src"val $lhs = scala.util.Random.nextLong() % $max")
      }
    case FixRandom(None) if !hw =>
      lhs.tp match {
