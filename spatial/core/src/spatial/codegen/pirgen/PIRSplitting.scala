...
trait PIRSplitting extends PIRTraversal {
  ...
  def nMems(groups: BusGroups, others: Iterable[CU]): Int = {
    val scalarGrps = if (groups.scalars.nonEmpty) {
      val producers = groups.scalars.groupBy { bus =>
        others.find { cu =>
          scalarOutputs(cu) contains bus
        }
      }
      producers.values.map { ss =>
        Math.ceil(ss.size.toDouble / SCALARS_PER_BUS).toInt
      }.sum
    } else 0
  ...
  def reportStats(stats: SplitStats) {
    val SplitStats(cus, ccus, ucus, alus, mems, sclIn, sclOut, vecIn, vecOut) =
      stats
  ...
  case class SplitCost(
    ...
    def >(that: SplitCost) =
      (this.sIn > that.sIn || this.vIn > that.vIn || this.vOut > that.vOut || this.vLoc > that.vLoc ||
        this.comp > (that.comp + that.write) || this.write > that.write ||
        this.read > that.read || this.mems > that.mems)

    def toStat =
      SplitStats(alus = comp + write,
                 mems = this.mems,
                 sclIn = sIn,
                 sclOut = sOut,
                 vecIn = vIn,
    ...
    override def toString =
      s"  sIn: $sIn, sOut: $sOut, vIn: $vIn, vOut: $vOut, vLoc: $vLoc, " +
  ...
  class Partition(write: Map[Int, WriteGroup],
                  compute: ArrayBuffer[Stage],
                  mems: Set[CUMemory],
                  owners: Set[LocalRef],
                  cc: Option[CUCChain],
                  edge: Boolean) {
    ...
    def allStages: Iterator[Stage] =
      cstages.iterator ++ wstages.values.flatMap(_.stages.iterator)

    def spliceSRAMOwner(): Boolean = {
      val ownerIdx = cstages.indexWhere {
        case stage @ MapStage(PIRBypass, ins, outs) =>
          !outs.exists {
            case LocalRef(_, SRAMReadReg(_)) => true; case _ => false
          } &&
            (ins exists (sramOwners contains _))

        case stage @ MapStage(op, ins, _) =>
          (ins exists (sramOwners contains _))
        case _ => false
      }
      if (ownerIdx > -1) {
        ...
        stage match {
          case stage @ MapStage(op, ins, outs) =>
            stage.ins = stage.ins.map {
              case `owner` => output; case ref => ref
            }
          case _ =>
            throw new Exception("Cannot have ReduceStage as SRAM owner")
        }
    ...
    private def add(writes: Map[Int, WriteGroup]) {
      ...
      val joined = keys.map { k =>
        (wstages.get(k), writes.get(k)) match {
          case (Some(a), Some(b)) =>
            k -> WriteGroup((a.mems ++ b.mems).distinct, a.stages)
          case (Some(a), None) => k -> a
          case (None, Some(b)) => k -> b
          case (None, None) =>
            throw new Exception(s"Not a possible combination")
        }
      }.toMap
      wstages = joined
    }
  ...
  object Partition {
    def empty(owners: Set[LocalRef], cc: Option[CUCChain], isEdge: Boolean) = {
      new Partition(Map[Int, WriteGroup](),
                    ArrayBuffer[Stage](),
                    Set[CUMemory](),
                    owners,
                    cc,
  ...
  def recomputeOwnedSRAMs(p: Partition, sramOwners: Set[LocalRef]) = {
    ...
    def writeStages(inputs: Set[LocalRef]) = {
      ...
      val srams = owners.collect {
        case LocalRef(_, SRAMReadReg(sram)) => sram
      }
  ...
  def recomputeOwnedCChains(
      p: Partition, /*control: Iterable[Stage],*/ ctrl: Option[CUCChain],
      isEdge: Boolean) = {
  ...
  def partitionCost(p: Partition,
                    prev: ArrayBuffer[Partition],
                    all: List[Stage],
                    others: Iterable[CU],
                    isUnit: Boolean) = {
    dbg(s"\n\n")
    ...
    for ((i, grp) <- p.wstages) {
      ...
      grp.stages.foreach { stage =>
        dbg(s"      $stage")
      }
    }
    dbg(s"  Compute stages: ")
    p.cstages.foreach { stage =>
      dbg(s"    $stage")
    }

    dbg(s"  CChains: ")
    p.cchains.foreach { cc =>
      dbg(s"    $cc :: " + globalInputs(cc).mkString(", "))
    }

    val local  = p.allStages.toList
    val remote = all diff local

    val localIns: Set[LocalComponent] = local
      .flatMap(_.inputMems)
      .toSet filterNot (isControl(_))
    ...
    val localReads: Set[CUMemory] = localIns.collect {
      case SRAMReadReg(sram) => sram
    }
    val remoteReads: Set[CUMemory] = remoteIns.collect {
      case SRAMReadReg(sram) => sram
    }
    // Inserted bypass reads
    val localBypasses: Set[CUMemory] = localOuts.collect {
      case SRAMReadReg(sram) => sram
    }
    val remoteBypasses: Set[CUMemory] = remoteOuts.collect {
      case SRAMReadReg(sram) => sram
    }

    val localWrites: Set[CUMemory] = localOuts.collect {
      case FeedbackDataReg(sram) => sram
    }
    val localAddrs: Set[CUMemory] = localOuts.collect {
      case FeedbackAddrReg(sram) => sram; case ReadAddrWire(sram) => sram
    }

    val remoteAddrs = remoteOuts.collect {
      case reg: FeedbackAddrReg => reg; case reg: ReadAddrWire => reg
    }

    // --- CU inputs and outputs
    val cuInBuses = globalInputs(localIns) ++ globalInputs(p.srams) ++ globalInputs(
      p.cchains)
    ...
    var sOuts: Int = localOuts.count {
      case ScalarOut(_) => true; case _ => false
    }
    ...
    p.srams.foreach { sram =>
      dbg(s"      $sram :: " + globalInputs(sram).mkString(", "))
    }
    ...
    dbg(
      s"    Remotely hosted, locally read: " + remoteHostLocalRead.mkString(
        ", "))
    ...
    dbg(
      s"    Locally hosted, remotely read: " + localHostRemoteRead.mkString(
        ", "))
    ...
    dbg(
      s"    Locally hosted, locally written: " + localHostLocalWrite.mkString(
        ", "))
    ...
    dbg(
      s"    Remotely hosted, locally written: " + remoteHostLocalWrite
        .mkString(", "))
    ...
    dbg(
      s"    Remotely hosted, locally addressed: " + remoteHostLocalAddr
        .mkString(", "))
    ...
    dbg(
      s"    Locally hosted, remotely addressed: " + remotelyAddressed.mkString(
        ", "))

    // --- Registers
    dbg(s"  Arg ins: " + cuGrpsIn.args.mkString(", "))

    val scalars = cuGrpsIn.scalars.map { bus =>
      s"$bus [" + others
        .find { cu =>
          scalarOutputs(cu) contains bus
        }
    ...
    if (!isUnit) {
      ...
      liveIns.foreach { in =>
        addIn(prev.indexWhere { part =>
          localOutputs(part.allStages) contains in
        })
      }
    }
    ...
    val reads = p.cstages.lastIndexWhere { stage =>
      stage.outputMems.exists(_.isInstanceOf[ReadAddrWire])
    } + 1
  ...
  def getSRAMOwners(stageGroups: Seq[Seq[Stage]]): Set[LocalRef] = {
    ...
    def sramFromRef(x: LocalRef) = x match {
      case LocalRef(_, SRAMReadReg(sram)) => Some(sram); case _ => None
    }
    def sramRefs(x: Seq[LocalRef]) = x.collect {
      case ref @ LocalRef(_, SRAMReadReg(_)) => ref
    }

    // Group set of stages into references for each memory
    def groupByMem(stages: Seq[Stage]): Seq[Seq[LocalRef]] =
      stages
        .flatMap { stage =>
          sramRefs(stage.inputRefs)
        }
        .groupBy(sramFromRef)
        .values
        .toSeq
    // Get reference with lowest stage number
    def getMemOwner(refs: Seq[LocalRef]): LocalRef = refs.reduce {
      (refA, refB) =>
        if (refA.stage < refB.stage) refA else refB
    }

    stageGroups
      .map { stages: Seq[Stage] =>
        groupByMem(stages)
      }
      .flatMap { mems =>
        mems.map { refs =>
  ...
  def splitCU(cu: CU, arch: SplitCost, others: Iterable[CU]): List[CU] = {
    val writeGroups = Map[Int, WriteGroup]() ++ cu.writeStages.zipWithIndex
      .map { case ((mems, stages), i) => i -> WriteGroup(mems, stages.toList) }
    val sramOwners = getSRAMOwners(
      cu.computeStages.toList +: cu.writeStages.values.map(_.toList).toList)

    dbg("\n\n\n")
    dbg(s"Splitting CU: $cu")
    dbg(s"Compute: ")
    cu.computeStages.foreach { stage =>
      dbg(s"  $stage")
    }
    dbg(s"Write Groups: ")
    writeGroups.foreach {
    ...
    val ctrl = cu.cchains.find {
      case _: UnitCChain | _: CChainInstance => true; case _ => false
    }

    val partitions         = ArrayBuffer[Partition]()
    var current: Partition = Partition.empty(sramOwners, ctrl, true)
    val remote: Partition = new Partition(writeGroups,
                                          cu.computeStages,
                                          cu.srams,
                                          sramOwners,
                                          ctrl,
    ...
    def getCost(p: Partition) =
      partitionCost(p, partitions, allStages, others, isUnit)
    ...
    val parent = if (partitions.length > 1) {
      val parent = ComputeUnit(cu.name,
                               cu.pipe,
                               (if (cu.isUnit) UnitStreamCU else StreamCU))
    ...
    val cus = partitions.zipWithIndex.map {
      case (p, i) =>
        schedulePartition(orig = cu, p, i, parent)
    }

    cus.zip(partitions).zipWithIndex.foreach {
      case ((cu, p), i) =>
  ...
  def schedulePartition(orig: CU,
                        part: Partition,
                        i: Int,
                        parent: Option[CU]): CU = {
    val style =
      if (parent.isDefined && orig.isUnit) UnitStreamCU
      else if (parent.isDefined) StreamCU
      else orig.style
    ...
    cu.writeStages ++= part.wstages.map {
      case (i, WriteGroup(mems, stages)) => mems -> ArrayBuffer(stages: _*)
    }
    cu.regs ++= cu.writeStages.values.flatMap(stages =>
      stages.flatMap(_.inputMems) ++ stages.flatMap(_.outputMems))
    ...
    def globalBus(reg: LocalComponent, isScalar: Boolean): GlobalBus =
      reg match {
        ...
        case SRAMReadReg(mem) =>
          val bus =
            if (isScalar) CUScalar(mem.name + "_data")
            else CUVector(mem.name + "_data")
          globals += bus
          bus
        case FeedbackAddrReg(mem) =>
          val bus =
            if (isScalar) CUScalar(mem.name + "_addr")
            else CUVector(mem.name + "_addr")
          globals += bus
          bus
        ...
        case _ =>
          val bus =
            if (isScalar) CUScalar("bus_" + reg.id)
            else CUVector("bus_" + reg.id)
          globals += bus
          bus
      }

    def portOut(reg: LocalComponent, isScalar: Boolean) =
      globalBus(reg, isScalar) match {
        case bus: ScalarBus => ScalarOut(bus)
        case bus: VectorBus => VectorOut(bus)
      }
    def portIn(reg: LocalComponent, isScalar: Boolean) =
      globalBus(reg, isScalar) match {
        case bus: ScalarBus => ScalarIn(bus)
        case bus: VectorBus => VectorIn(bus)
      }

    def rerefIn(reg: LocalComponent,
                isScalar: Boolean = orig.isUnit): LocalRef = {
      val in = reg match {
        case _: ConstReg | _: CounterReg => reg
        case _: ValidReg | _: ControlReg => reg
        case _: ReduceMem[_] =>
          if (localOuts.contains(reg)) reg else portIn(reg, isScalar)
        case SRAMReadReg(sram) =>
          if (cu.srams.contains(sram)) reg else portIn(reg, isScalar)
        case _ => if (cu.regs.contains(reg)) reg else portIn(reg, isScalar)
      }
      cu.regs += in
      ctx.refIn(in)
    }
    def rerefOut(reg: LocalComponent,
                 isScalar: Boolean = orig.isUnit): List[LocalRef] = {
      val outs = reg match {
        case _: ScalarOut => List(reg)
        case _: VectorOut => List(reg)
        case FeedbackAddrReg(sram) =>
          if (cu.srams.contains(sram)) List(reg)
          else List(portOut(reg, isScalar))
        case FeedbackDataReg(sram) =>
          if (cu.srams.contains(sram)) List(reg)
          else List(portOut(reg, isScalar))
        case ReadAddrWire(sram) =>
          if (cu.srams.contains(sram)) List(reg)
          else List(portOut(reg, isScalar))
        ...
        case _ =>
          ...
          val global =
            if (remoteIns.contains(reg)) List(portOut(reg, isScalar)) else Nil
          local ++ global
      }
      cu.regs ++= outs
      outs.map { out =>
    ...
    cu.srams.foreach { sram =>
      val remoteAddr = remoteOuts.find {
        case ReadAddrWire(`sram`) => true; case _ => false
      }
      val localAddr = localOuts.find {
        case ReadAddrWire(`sram`) => true; case _ => false
      }
      ...
      if (remoteAddr.isDefined && localAddr.isEmpty) {
        ...
        ctx.addStage(
          MapStage(PIRBypass, List(ctx.refIn(addrIn)), List(ctx.refOut(reg))))
        cu.regs += reg
        cu.regs += addrIn
      }
    }
    ...
    part.cstages.foreach {
      case MapStage(op, ins, outs) =>
        val inputs = ins.map { in =>
          rerefIn(in.reg)
        }
        val outputs = outs.flatMap { out =>
          rerefOut(out.reg)
        }
      ...
      case ReduceStage(op, init, in, acc) =>
        ...
        if (remoteIns.contains(acc)) {
          ...
          ctx.addStage(
            MapStage(PIRBypass, List(ctx.refIn(acc)), List(ctx.refOut(bus))))
        }
    }
    ...
    localBypasses.foreach { sram =>
      ...
      if (!cu.computeStages.flatMap(_.outputMems).contains(out)) {
        ctx.addStage(
          MapStage(PIRBypass, List(ctx.refIn(reg)), List(ctx.refOut(out))))
        cu.regs += reg
        cu.regs += out
      }
    }
    ...
    if (parent.isDefined) {
      ...
      cu.cchains = cu.cchains.filter { cc =>
        part.cchains.exists { _.name == cc.name }
      }
      ...
      def tx(cc: CUCChain): CUCChain = {
        if (f.contains(cc)) f(cc)
          ...
          throw new Exception(
            s"Attempted to copy counter $cc in CU $ctrl, but no such counter exists.\nMapping:\n$mapping")
        }
      }
      ...
      cu.allStages.foreach {
        case stage @ MapStage(_, ins, _) =>
          stage.ins = ins.map { swap_cchains_Ref(_) }
        case _ =>
      }
      cu.srams.foreach { sram =>
        sram.readAddr = sram.readAddr.map {
          swap_cchain_Reg(_).asInstanceOf[ReadAddr]
        } //TODO refactor this
        sram.writeAddr = sram.writeAddr.map {
          swap_cchain_Reg(_).asInstanceOf[WriteAddr]
        }
