...
trait ParameterAnalyzer extends SpatialTraversal {
  ...
  def onlyIndex(x: Seq[Const[_]]): Seq[Param[Index]] = {
    x.collect {
      case p: Param[_] if isInt32Type(p.tp) => p.asInstanceOf[Param[Index]]
    }
  }

  def setRange(x: Param[Index], min: Int, max: Int, stride: Int = 1): Unit =
    domainOf(x) match {
      case Some((start, end, step)) =>
        domainOf(x) =
          (Math.max(min, start), Math.min(max, end), Math.max(stride, step))
      case None =>
  ...
  override protected def postprocess[S: Type](block: Block[S]) = {
    ...
    params.foreach { p =>
      if (domainOf(p).isEmpty) domainOf(p) = (1, 1, 1)
    }
  ...
  override protected def visit(lhs: Sym[_], rhs: Op[_]) = rhs match {
    case FIFONew(c @ Exact(_)) =>
      warn(
        u"FIFO $lhs has parametrized size. Tuning of FIFO depths is not yet supported.")
    ...
    case SRAMNew(dims) =>
      ...
      (tsizes intersect dims).foreach { p =>
        if (dims.indexOf(p) == dims.length - 1)
    ...
    case e: OpForeach =>
      ...
      if (!isParallelizableLoop(lhs)) pars.foreach { p =>
        setRange(p, 1, 1)
      } else if (!isInnerPipe(lhs)) pars.foreach { p =>
        setRange(p, 1, MAX_OUTER_PAR)
      }
    ...
    case e: OpMemReduce[_, _] =>
      ...
      if (!isParallelizableLoop(lhs)) opars.foreach { p =>
        setRange(p, 1, 1)
      }
