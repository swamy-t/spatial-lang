...
trait PIRDSE extends PIRSplitting with PIRRetiming {
  ...
  def dse() {
    ...
    invalid.println(
      "Scl/Bus, SIns, VIns, Vouts, Compute, Read/Write, Stages, SRAMs")
    valid.println(
      "Scl/Bus, SIns, VIns, Vouts, Compute, Read/Write, Stages, SRAMs," + header.heading +
    ...
    for (vIns      <- 2 to 6; // 5
      ...
      try {
          ...
          for (cu <- split) {
            ...
            if (cost.mems > max.mems)
              throw new SplitException(
                s"$orig requires ${cost.mems} > ${max.mems} SRAMs after retiming")
        ...
        if (pass == 0) {
          first =
            s"sbus=$sbus, sIn=$sIns, vIn=$vIns, vOut=$vOuts, comps=$comps, read/write=$readWrite, mems=$mmems"
        }
        pass += 1

        System.out.println(
          s"sbus=$sbus, sIn=$sIns, vIn=$vIns, vOut=$vOuts, comps=$comps, read/write=$readWrite, mems=$mmems: PASS")
        valid.println(
          s"$sbus, $sIns, $vIns, $vOuts, $comps, $readWrite, $stages, $mmems, " + stats.toString +
        ...
        case e: SplitException =>
          fail += 1
          System.out.println(
            s"sbus=$sbus, sIn=$sIns, vIn=$vIns, vOut=$vOuts, comps=$comps, read/write=$readWrite, mems=$mmems: FAIL")
          System.out.println(e.msg)
          invalid.println(
            s"$sbus, $sIns, $vIns, $vOuts, $comps, $readWrite, $stages, $mmems")
      }
    }
    valid.close()
    invalid.close()
