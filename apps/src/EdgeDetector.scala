...
object EdgeDetector extends SpatialApp {
  ...
  def main() {
    ...
    val rowtile = 16
    val coltile = 64
    val data = loadCSV2D[T](
      "/home/mattfel/spatial-lang/spatial/core/resources/cppgen/testdata/slacsample2d.csv",
      ",",
      "\n")
    ...
    Accel {
      ...
      Sequential.Foreach(memrows by rowtile) { r =>
        Sequential.Foreach(rowtile by 1) { rr =>
          ...
          val globalMax =
            Reduce(Reg[Tup2[Int, T]](pack(0.to[Int], -1000.to[T])))(
              memcols by coltile) { c =>
              // Load tile from row
              ...
              val localMax =
                Reduce(Reg[Tup2[Int, T]](pack(0.to[Int], -1000.to[T])))(
                  coltile by 1) { j =>
                  sr(1, *) <<= rawdata(j)
                  val mean_right = Reduce(Reg[T](0.to[T]))(window / 2 by 1) {
                    k =>
                      sr(0, k)
                  } { _ + _ } / window.to[T]
                  val mean_left = Reduce(Reg[T](0.to[T]))(window / 2 by 1) {
                    k =>
                      sr(0, k + window / 2)
                  } { _ + _ } / window.to[T]
...
object Differentiator extends SpatialApp {
  ...
  def main() {
    type T = FixPt[TRUE, _16, _16]
    val coltile = 64
    val data = loadCSV1D[T](
      "/home/mattfel/spatial-lang/spatial/core/resources/cppgen/testdata/slacsample1d.csv",
      ",")
    val memcols = ArgIn[Int]
    ...
    Accel {
      ...
      Foreach(memcols by coltile) { c =>
        ...
        Foreach(coltile by 1) { j =>
          sr(1, *) <<= rawdata(j)
          val mean_right = Reduce(Reg[T](0.to[T]))(window / 2 by 1) { k =>
            sr(0, k)
          } { _ + _ } / window.to[T]
          val mean_left = Reduce(Reg[T](0.to[T]))(window / 2 by 1) { k =>
            sr(0, k + window / 2)
          } { _ + _ } / window.to[T]
          val slope = (mean_right - mean_left) / (window / 2).to[T]
          val idx   = j + c
