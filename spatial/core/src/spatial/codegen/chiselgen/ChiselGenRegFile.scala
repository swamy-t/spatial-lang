...
trait ChiselGenRegFile extends ChiselGenSRAM {
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    case op @ RegFileNew(dims) =>
      ...
      emitGlobal(
        s"val ${quote(lhs)} = Module(new templates.ParallelShiftRegFile(${dims(
          0)}, ${dims(1)}, 1, ${par}/${dims(0)}))")
      emitGlobal(s"${quote(lhs)}.io.reset := reset")
    // cols = dims(1).toInt

    case op @ RegFileLoad(rf, inds, en) =>
      if (needsFPType(lhs.tp)) {
        lhs.tp match {
          case FixPtType(s, d, f) =>
            emit(src"""val ${lhs} = Wire(new FixedPoint($s, $d, $f))""")
            emit(
              src"""${lhs}.number := ${rf}.readValue(${inds(0)}.number, ${inds(
                1)}.number)""")
          case _ =>
            emit(
              src"""${lhs} := ${rf}.readValue(${inds(0)}.number, ${inds(1)}.number)""")
        }
      } else {
        emit(
          src"""${lhs} := ${rf}.readValue(${inds(0)}.number, ${inds(1)}.number)""")
    ...
    case op @ RegFileStore(rf, inds, data, en) =>
      ...
      emit(
        s"${quote(rf)}.connectWPort(${quote(data)}.number, ${quote(inds(0))}.number, ${quote(
          inds(1))}.number, ${quote(en)} & ${quote(parent)}_datapath_en)")
    // TODO: finish this using inds like in Load

    case RegFileShiftIn(rf, inds, d, data, en) =>
      val parent = writersOf(rf).find { _.node == lhs }.get.ctrlNode
      emit(s"${quote(rf)}.connectShiftPort(${quote(data)}.number, ${quote(
        inds(0))}.number, ${quote(en)} & ${quote(parent)}_datapath_en)")
