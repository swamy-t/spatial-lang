...
trait SimGenFltPt extends SimCodegen {
  ...
  override protected def quoteConst(c: Const[_]): String = (c.tp, c) match {
    case (FltPtType(g, e), Const(c: BigDecimal)) if hw =>
      src"Number($c, true, FloatPoint($g,$e))"
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case FltConvert(x) if hw =>
      lhs.tp match {
        case FltPtType(g, e) =>
          delay(lhs, rhs, src"Number($x.value, $x.valid, FloatPoint($g,$e))")
      }
    ...
    case FltRandom(Some(max)) if !hw =>
      lhs.tp match {
        case FloatType() =>
          emit(src"val $lhs = scala.util.Random.nextFloat() * $max")
        case DoubleType() =>
          emit(src"val $lhs = scala.util.Random.nextDouble() * $max")
      }
    case FltRandom(None) if !hw =>
      lhs.tp match {
        case FloatType() => emit(src"val $lhs = scala.util.Random.nextFloat()")
        case DoubleType() =>
          emit(src"val $lhs = scala.util.Random.nextDouble()")
      }
