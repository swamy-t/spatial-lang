...
trait CounterExp { this: SpatialExp =>
  ...
  /** Direct methods **/
  @api def CounterChain(counters: Counter*): CounterChain =
    CounterChain(counterchain_new(unwrap(counters)))
  @api def Counter(end: Index): Counter =
    counter(0, end, 1, Some(wrap(intParam(1))))
  @api def Counter(start: Index, end: Index): Counter =
    counter(start, end, 1, Some(wrap(intParam(1))))
  @api def Counter(start: Index, end: Index, step: Index): Counter =
    counter(start, end, step, Some(wrap(intParam(1))))
  @api
  def Counter(start: Index, end: Index, step: Index, par: Index): Counter =
  ...
  implicit def wildcard2counter(wild: Wildcard)(
      implicit ctx: SrcCtx): Counter = forever

  def extractParFactor(par: Option[Index])(
      implicit ctx: SrcCtx): Const[Index] = par.map(_.s) match {
  ...
  def counter(start: Index, end: Index, step: Index, par: Option[Index])(
      implicit ctx: SrcCtx): Counter = {
  ...
  /** IR Nodes **/
  case class CounterNew(start: Exp[Index],
                        end: Exp[Index],
                        step: Exp[Index],
                        par: Const[Index])
      extends Op[Counter] {
  ...
  case class CounterChainNew(counters: Seq[Exp[Counter]])
      extends Op[CounterChain] {
    def mirror(f: Tx) = counterchain_new(f(counters))
  }

  case class Forever() extends Op[Counter] {
    def mirror(f: Tx) = forever_counter()
  }

  /** Constructors **/
  def counter_new(start: Exp[Index],
                  end: Exp[Index],
                  step: Exp[Index],
                  par: Const[Index])(implicit ctx: SrcCtx): Sym[Counter] = {
  ...
  def counterchain_new(counters: Seq[Exp[Counter]])(implicit ctx: SrcCtx) =
    stageCold(CounterChainNew(counters))(ctx)
  ...
  def isForeverCounterChain(x: Exp[CounterChain]): Boolean =
    countersOf(x).exists(isForever)
  def isUnitCounterChain(x: Exp[CounterChain]): Boolean =
    countersOf(x).forall(isUnitCounter)
}
