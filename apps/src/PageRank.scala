...
object PageRank extends SpatialApp {
  ...
  def main() {
    ...
    val pages = Array.tabulate(NP) { i =>
      random[X](3)
    }
    val edges = Array
      .tabulate(NP) { i =>
        Array.tabulate(edges_per_page) { j =>
          if (i < edges_per_page) j else i - j
        }
      }
      .flatten
    val counts = Array
      ...
      .flatten
    val edgeId = Array.tabulate(NP) { i =>
      i * edges_per_page
    }
    val edgeLen = Array.tabulate(NP) { i =>
      edges_per_page.to[Int]
    }

    val result =
      pagerank(pages, edges, counts, edgeId, edgeLen, iters, damp, NP)
    ...
    for (ep <- 0 until iters) {
      for (i <- 0 until NP) {
        ...
        val iterator = Array.tabulate(numEdges) { kk =>
          startId + kk
        }
        val these_edges = iterator.map { j =>
          edges(j)
        }
        val these_pages = these_edges.map { j =>
          gold(j)
        }
        val these_counts = these_edges.map { j =>
          counts(j)
        }
        val pr = these_pages
          .zip(these_counts) { (p, c) =>
