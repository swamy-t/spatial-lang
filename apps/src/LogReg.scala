...
object LogReg extends SpatialApp {
  ...
  @virtualize
  def logreg[T: Type: Num](xIn: Array[T],
                           yIn: Array[T],
                           tt: Array[T],
                           n: Int,
                           it: Int) = {
        ...
        Sequential.MemReduce(btheta)(1 by 1) { xx =>
          ...
          Foreach(N by BN) { i =>
            ...
            MemReduce(gradAcc)(BN par P3) { ii =>
              ...
              val dotAccum = Reduce(Reg[T])(D par P2) { j =>
                logregX(ii, j) * btheta(j)
              } { _ + _ } // read
              Pipe { pipe2Res := (logregY(ii) - sigmoid(dotAccum.value)) }
              Foreach(D par P2) { j =>
                subRam(j) = logregX(ii, j) - pipe2Res.value
              }
              subRam
            } { _ + _ }
          }
