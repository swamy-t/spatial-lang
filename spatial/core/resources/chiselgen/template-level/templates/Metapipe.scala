...
class Metapipe(val n: Int, val isFSM: Boolean = false) extends Module {
  ...
  when(io.input.enable) {
    when(state === initState.U) { // INIT -> RESET
      ...
      io.output.stageEnable.foreach { s =>
        ...
        stateFF.io.input.data := Mux(
          io.input.numIter === 0.U,
          Mux(io.input.forever, steadyState.U, doneState.U),
          fillState.U) // Go directly to done if niters = 0
        io.output.stageEnable.foreach { s =>
          s := false.B
        }
      }
      .elsewhen(state < steadyState.U) { // FILL -> STEADY
        for (i <- fillState until steadyState) {
          ...
          when((state === i.U)) {
            ...
            io.output.stageEnable.drop(fillStateID + 1).foreach { en =>
              en := false.B
            }
            val doneMaskInts = doneMask.take(fillStateID + 1).map {
              Mux(_, 1.U(bitsToAddress(n).W), 0.U(bitsToAddress(n).W))
            }
            ...
            when(doneTree === 1.U) {
              if (i + 1 == steadyState) { // If moving to steady state
                stateFF.io.input.data := Mux(
                  cycsSinceDone.io.output.data === 0.U & ctr.io.output
                    .count(0) + 1.U < max,
                  steadyState.U,
                  Mux(
                    io.input.forever,
                    steadyState.U,
                    cycsSinceDone.io.output.data + 2.U + stateFF.io.output.data)
      ...
      .elsewhen(state === steadyState.U) { // STEADY
        io.output.stageEnable.zip(doneMask).foreach {
          case (en, done) => en := ~done
        }
      ...
      .elsewhen(state < doneState.U) { // DRAIN
        for (i <- drainState until doneState) {
          ...
          when(state === i.U) {
            io.output.stageEnable
              .zip(doneMask)
              .takeRight(n - drainStateID - 1)
              .foreach { case (en, done) => en := ~done }
            io.output.stageEnable.dropRight(n - drainStateID - 1).foreach {
              en =>
                en := 0.U
            }

            val doneTree =
              doneMask.takeRight(n - drainStateID - 1).reduce { _ & _ }
            doneClear := doneTree
            when(doneTree === 1.U) {
              stateFF.io.input.data := (i + 1).U
            }.otherwise {
  ...
  // Output logic
  io.output.ctr_inc := io.input.stageDone(0) & Utils.delay(
    ~io.input.stageDone(0),
    1) // on rising edge
  io.output.done := state === doneState.U
}

class Streampipe(override val n: Int, override val isFSM: Boolean = false)
    extends Parallel(n) {}
