...
trait ScalarAnalyzer extends SpatialTraversal {
  ...
  def checkForGlobals(lhs: Sym[_], rhs: Op[_]): Unit = lhs match {
    ...
    case _ =>
      if (isPrimitiveNode(lhs) && rhs.inputs.nonEmpty && rhs.inputs.forall(
            isGlobal(_)))
        isGlobal(lhs) = true
  }
  ...
  def analyzeBounds(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case RegWrite(reg @ Bounded(b1), Bounded(b2), _) if !insideLoop =>
      boundOf(reg) = b1 meet b2
    ...
    case IfThenElse(c, thenp, elsep) =>
      (thenp.result, elsep.result) match {
    ...
    case FixDiv(Final(a), Final(b)) =>
      boundOf(lhs) = Exact(a / b + (if ((a mod b) > 0) 1 else 0))
    case FixDiv(Exact(a), Exact(b)) =>
      boundOf(lhs) = Exact(a / b + (if ((a mod b) > 0) 1 else 0))
    case FixDiv(Bound(a), Bound(b)) =>
      boundOf(lhs) = Bound(a / b + (if ((a mod b) > 0) 1 else 0))
    case _ =>
  }
