...
trait NodeClasses { this: SpatialExp =>
  ...
  def isControlNode(e: Exp[_]): Boolean =
    isPipeline(e) || isParallel(e) || isDRAMTransfer(e) || isSwitch(e) || isSwitchCase(
      e)
  def isOuterControl(e: Exp[_]): Boolean =
    isControlNode(e) && levelOf(e) == OuterControl
  def isInnerControl(e: Exp[_]): Boolean =
    isControlNode(e) && levelOf(e) == InnerControl
  ...
  def willRunForever(e: Exp[_]): Boolean =
    getDef(e).exists(isForever) || childrenOf(e).exists(willRunForever)
  ...
  def isForever(d: Def): Boolean = d match {
    ...
    case e: OpMemReduce[_, _] =>
      isForeverCounterChain(e.cchainMap) // This should probably never happen
  ...
  def isDynamicAllocation(e: Exp[_]): Boolean =
    getDef(e).exists(isDynamicAllocation)
  ...
  def isPrimitiveAllocation(e: Exp[_]): Boolean =
    getDef(e).exists(isPrimitiveAllocation)
  ...
  def isLocalMemory(e: Exp[_]): Boolean = e.tp match {
    case _: SRAMType[_] | _: FIFOType[_] | _: RegType[_] |
        _: LineBufferType[_] | _: RegFileType[_] =>
      true
    case _: StreamInType[_]  => true
    case _: StreamOutType[_] => true
    case _                   => false
  ...
  /** Host Transfer **/
  def isTransfer(e: Exp[_]): Boolean =
    isTransferToHost(e) || isTransferFromHost(e)
  ...
  def isTransferFromHost(e: Exp[_]): Boolean =
    getDef(e).exists(isTransferFromHost)
  ...
  def isPrimitiveNode(e: Exp[_]): Boolean = e match {
    ...
    case Param(_) => false
    case _ =>
      !isControlNode(e) && !isAllocation(e) && !isStateless(e) && !isGlobal(e) && !isFringeNode(
        e)
  }
  ...
  type LocalWrite =
    (Exp[_], Option[Exp[_]], Option[Seq[Exp[Index]]], Option[Exp[Bool]])
  ...
  private object LocalRead {
    ...
    def apply(mem: Exp[_],
              addr: Seq[Exp[Index]] = null,
              en: Exp[Bool] = null): List[LocalRead] = {
      List((mem, Option(addr), Option(en)))
    }
  }

  def writerUnapply(d: Def): Option[List[LocalWrite]] = d match {
    case RegWrite(reg, data, en) =>
      Some(LocalWrite(reg, value = data, en = en))
    case RegFileStore(reg, inds, data, en) =>
      Some(LocalWrite(reg, value = data, addr = inds, en = en))
    case SRAMStore(mem, _, inds, _, data, en) =>
      Some(LocalWrite(mem, value = data, addr = inds, en = en))
    case FIFOEnq(fifo, data, en) =>
      Some(LocalWrite(fifo, value = data, en = en))

    case RegFileShiftIn(reg, is, d, data, en) =>
      Some(LocalWrite(reg, value = data, addr = is, en = en))
    case ParRegFileShiftIn(reg, is, d, data, en) =>
      Some(LocalWrite(reg, value = data, addr = is, en = en))

    case LineBufferEnq(lb, data, en) =>
      Some(LocalWrite(lb, value = data, en = en))

    case e: DenseTransfer[_, _] if e.isLoad =>
      Some(LocalWrite(e.local, addr = e.iters))
    case e: SparseTransfer[_] if e.isLoad =>
      Some(LocalWrite(e.local, addr = Seq(e.i)))

    case StreamWrite(stream, data, en) =>
      Some(LocalWrite(stream, value = data, en = en))
  ...
  def readerUnapply(d: Def): Option[List[LocalRead]] = d match {
    ...
    case RegFileLoad(reg, inds, en) =>
      Some(LocalRead(reg, addr = inds, en = en))
    case SRAMLoad(mem, dims, inds, ofs, en) =>
      Some(LocalRead(mem, addr = inds, en = en))
    case FIFODeq(fifo, en) => Some(LocalRead(fifo, en = en))

    case LineBufferLoad(lb, row, col, en) =>
      Some(LocalRead(lb, addr = Seq(row, col), en = en))
    case LineBufferColSlice(lb, row, col, len) =>
      Some(LocalRead(lb, addr = Seq(row, col)))
    case LineBufferRowSlice(lb, row, len, col) =>
      Some(LocalRead(lb, addr = Seq(row, col)))

    case e: DenseTransfer[_, _] if e.isStore =>
      Some(LocalRead(e.local, addr = e.iters))
    case e: SparseTransfer[_] if e.isLoad => Some(LocalRead(e.addrs))
    case e: SparseTransfer[_] if e.isStore =>
      Some(LocalRead(e.addrs) ++ LocalRead(e.local))
  ...
  object LocalWriter {
    def unapply(x: Exp[_]): Option[List[LocalWrite]] =
      getDef(x).flatMap(writerUnapply)
    def unapply(d: Def): Option[List[LocalWrite]] = writerUnapply(d)
  }
  object LocalReader {
    def unapply(x: Exp[_]): Option[List[LocalRead]] =
      getDef(x).flatMap(readerUnapply)
    def unapply(d: Def): Option[List[LocalRead]] = readerUnapply(d)
  }
  object LocalAccess {
    def unapply(x: Exp[_]): Option[List[Exp[_]]] =
      getDef(x).flatMap(LocalAccess.unapply)
