...
trait ChiselGenLineBuffer extends ChiselCodegen {
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    case op @ LineBufferNew(rows, cols) =>
      ...
      emitGlobal(
        s"""val ${quote(lhs)} = Module(new templates.LineBuffer($rows, $cols, 1, 
        ${col_wPar}, ${col_rPar}, 
    ...
    withStream(getStream("BufferControlCxns")) {
      linebufs.foreach { mem =>
        ...
        val readPorts = readers
          .filter { reader =>
            dispatchOf(reader, mem).contains(0)
          }
          .groupBy { a =>
            portsOf(a, mem, 0)
          }
        val writePorts = writers
          .filter { writer =>
            dispatchOf(writer, mem).contains(0)
          }
          .groupBy { a =>
            portsOf(a, mem, 0)
          }
        ...
        (0 to numStagesInbetween).foreach { port =>
          ...
          val wr =
            if (writePortsNumbers.toList.contains(ctrlId)) { "write" } else ""
          val empty = if (rd == "" & wr == "") "empty" else ""
          emit(
            src"""${mem}.connectStageCtrl(${quote(node)}_done, ${quote(node)}_en, List(${port})) /*$rd $wr $empty*/""")
        }

        emit(
          src"""${mem}.lockUnusedCtrl() // Specific method for linebufs, since there is one ctrl port per line, but possibly not an access per line """)
