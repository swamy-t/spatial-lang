...
case class AXI4SlavePortParameters(slaves: Seq[AXI4SlaveParameters],
                                   beatBytes: Int) {
  ...
  def findSafe(address: UInt) =
    Vec(slaves.map(_.address.map(_.contains(address)).reduce(_ || _)))
  def findFast(address: UInt) =
    Vec(
      slaves.map(
        _.address
          .map(_.widen(~routingMask))
          .distinct
...
case class AXI4MasterParameters(id: IdRange = IdRange(0, 1),
...
case class AXI4MasterPortParameters(masters: Seq[AXI4MasterParameters]) {
  val endId = masters.map(_.id.end).max

  // Require disjoint ranges for ids
  masters.combinations(2).foreach {
    case Seq(x, y) => require(!x.id.overlaps(y.id))
  }
}
...
case class AXI4BundleParameters(addrBits: Int, dataBits: Int, idBits: Int) {
  ...
  def union(x: AXI4BundleParameters) =
    AXI4BundleParameters(max(addrBits, x.addrBits),
...
object AXI4BundleParameters {
  def apply(master: AXI4MasterPortParameters, slave: AXI4SlavePortParameters) =
    new AXI4BundleParameters(addrBits = log2Up(slave.maxAddress + 1),
