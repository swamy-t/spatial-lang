...
class FromAccel(val p: Int) extends Bundle {
  ...
  override def cloneType =
    (new FromAccel(p)).asInstanceOf[this.type] // See chisel3 bug 358
}
class ToAccel(val p: Int) extends Bundle {
  ...
  override def cloneType =
    (new ToAccel(p)).asInstanceOf[this.type] // See chisel3 bug 358
}
class FromDRAM(val p: Int) extends Bundle {
  ...
  override def cloneType =
    (new FromDRAM(p)).asInstanceOf[this.type] // See chisel3 bug 358
}
class ToDRAM(val p: Int) extends Bundle {
  ...
  override def cloneType =
    (new ToDRAM(p)).asInstanceOf[this.type] // See chisel3 bug 358
}

class MemController(val pLoadAccel: Int,
                    val pStoreAccel: Int,
                    val pStoreDRAM: Int,
                    val pLoadDRAM: Int)
    extends Module {
  ...
  io.CtrlToDRAM.data.zip(io.AccelToCtrl.data).foreach {
    case (data, port) => data := port
  }
  io.CtrlToDRAM.size := io.AccelToCtrl.size

  io.CtrlToAccel.data.zip(io.DRAMToCtrl.data).foreach {
    case (data, port) => data := port
  }
  ...
  io.CtrlToAccel.cmdIssued := !loadFifo.io.empty & Utils.delay(
    loadFifo.io.empty,
    1) // TODO: May cause bug if fifo drains faster than it loads
