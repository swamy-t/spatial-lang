...
trait SpatialMetadataApi { this: SpatialApi =>
  ...
  object bound {
    @api def update[T: Type](x: T, value: scala.Long): Unit =
      setBound(x, BigInt(value))
  }
}
...
trait SpatialMetadataExp extends IndexPatternExp { this: SpatialExp =>
  ...
  /**
    ...
    **/
  case class MBound(bound: BigInt, isExact: Boolean, isFinal: Boolean)
      extends Metadata[MBound] {
  ...
  object boundOf {
    ...
    def update(x: Exp[_], value: Option[MBound]): Unit = value.foreach { v =>
      boundOf(x) = v
    }
    def get(x: Exp[_]): Option[BigInt] = metadata[MBound](x).map(_.bound)
    def apply(x: Exp[_]): BigInt       = boundOf.get(x).get
  }
  private[spatial] def setBound[T: Type](x: T, value: BigInt): Unit = {
    boundOf(x.s) = value
  }
  ...
  case class ControlType(style: ControlStyle) extends Metadata[ControlType] {
    def mirror(f: Tx) = this
  }
  object styleOf {
    def apply(x: Exp[_]): ControlStyle =
      styleOf.get(x).getOrElse { throw new UndefinedControlStyleException(x) }
    def update(x: Exp[_], style: ControlStyle): Unit =
      metadata.add(x, ControlType(style))
    def get(x: Exp[_]): Option[ControlStyle] =
      metadata[ControlType](x).map(_.style)
  }

  case class MControlLevel(level: ControlLevel)
      extends Metadata[MControlLevel] { def mirror(f: Tx) = this }
  object levelOf {
    def apply(x: Exp[_]): ControlLevel =
      levelOf.get(x).getOrElse { throw new UndefinedControlLevelException(x) }
    def update(x: Exp[_], level: ControlLevel): Unit =
      metadata.add(x, MControlLevel(level))
    def get(x: Exp[_]): Option[ControlLevel] =
      metadata[MControlLevel](x).map(_.level)
  }
  ...
  /**
    ...
    */
  case class ParamRange(min: Int, step: Int, max: Int)
      extends Metadata[ParamRange] { def mirror(f: Tx) = this }
  object domainOf {
    def apply(x: Param[Int32]): Option[(Int, Int, Int)] =
      metadata[ParamRange](x).map { d =>
        (d.min, d.step, d.max)
      }
    def update(x: Param[Int32], rng: (Int, Int, Int)) =
      metadata.add(x, ParamRange(rng._1, rng._2, rng._3))
  }
  ...
  /**
    ...
    */
  case class MGlobal(isGlobal: Boolean) extends Metadata[MGlobal] {
    def mirror(f: Tx) = this
  }
  object isGlobal {
  ...
  /**
    ...
    */
  case class SoftDims(dims: Seq[Exp[Index]]) extends Metadata[SoftDims] {
    def mirror(f: Tx) = SoftDims(f(dims))
  }
  object softDimsOf {
    def apply(x: Exp[_]): Seq[Exp[Index]] =
      metadata[SoftDims](x).map(_.dims).getOrElse(Nil)
    def update(x: Exp[_], dims: Seq[Exp[Index]]) =
      metadata.add(x, SoftDims(dims))
  }

  def mirrorCtrl(x: Ctrl, f: Tx): Ctrl = (f(x.node), x.isInner)
  def mirrorAccess(x: Access, f: Tx): Access =
    (f(x.node), mirrorCtrl(x.ctrl, f))
  ...
  object readersOf {
    def apply(x: Exp[_]): List[Access] =
      metadata[Readers](x).map(_.readers).getOrElse(Nil)
    def update(x: Exp[_], readers: List[Access]) =
      metadata.add(x, Readers(readers))
  }
  ...
  object writersOf {
    def apply(x: Exp[_]): List[Access] =
      metadata[Writers](x).map(_.writers).getOrElse(Nil)
    def update(x: Exp[_], writers: List[Access]) =
      metadata.add(x, Writers(writers))
  }
  ...
  /**
    ...
    **/
  case class Children(children: List[Exp[_]]) extends Metadata[Children] {
    def mirror(f: Tx) = Children(f.tx(children))
  }
  object childrenOf {
    def apply(x: Exp[_]): List[Exp[_]] =
      metadata[Children](x).map(_.children).getOrElse(Nil)
    def update(x: Exp[_], children: List[Exp[_]]) =
      metadata.add(x, Children(children))

    def apply(x: Ctrl): List[Ctrl] = {
      val children = childrenOf(x.node).map { child =>
        (child, false)
      }
  ...
  /**
    ...
    **/
  case class Parent(parent: Exp[_]) extends Metadata[Parent] {
    def mirror(f: Tx) = Parent(f(parent))
  }
  object parentOf {
    ...
    def apply(x: Ctrl): Option[Ctrl] =
      if (x.isInner) Some((x.node, false))
      else
        parentOf(x.node).map { x =>
          (x, false)
        }
  }

  case class CtrlDeps(deps: Set[Exp[_]]) extends Metadata[CtrlDeps] {
    def mirror(f: Tx) = CtrlDeps(f.tx(deps))
  }
  object ctrlDepsOf {
    def apply(x: Exp[_]): Set[Exp[_]] =
      metadata[CtrlDeps](x).map(_.deps).getOrElse(Set.empty)
  ...
  object writtenIn {
    def apply(x: Exp[_]): List[Exp[_]] =
      metadata[WrittenMems](x).map(_.written).getOrElse(Nil)
    def update(x: Exp[_], written: List[Exp[_]]): Unit =
      metadata.add(x, WrittenMems(written))

    def apply(x: Ctrl): List[Exp[_]] = writtenIn(x.node)
    def update(x: Ctrl, written: List[Exp[_]]): Unit =
      writtenIn(x.node) = written
  }

  /**
    * List of fifos or streams popped in a given controller, for handling streampipe control flow
    **/
  case class ListenStreams(listen: List[Exp[_]])
      extends Metadata[ListenStreams] {
    def mirror(f: Tx) = ListenStreams(f.tx(listen))
  }
  object listensTo {
    def apply(x: Exp[_]): List[Exp[_]] =
      metadata[ListenStreams](x).map(_.listen).getOrElse(Nil)
    def update(x: Exp[_], listen: List[Exp[_]]): Unit =
      metadata.add(x, ListenStreams(listen))

    def apply(x: Ctrl): List[Exp[_]] = listensTo(x.node)
    def update(x: Ctrl, listen: List[Exp[_]]): Unit =
      listensTo(x.node) = listen
  }
  ...
  object loadCtrlOf {
    def apply(x: Exp[_]): List[Exp[_]] =
      metadata[LoadMemCtrl](x).map(_.ctrl).getOrElse(Nil)
    def update(x: Exp[_], ctrl: List[Exp[_]]): Unit =
      metadata.add(x, LoadMemCtrl(ctrl))
  ...
  object pushesTo {
    def apply(x: Exp[_]): List[Exp[_]] =
      metadata[PushStreams](x).map(_.push).getOrElse(Nil)
    def update(x: Exp[_], push: List[Exp[_]]): Unit =
      metadata.add(x, PushStreams(push))
  ...
  /**
    ...
    */
  case class ArgMap(argId: Int, memIdIn: Int, memIdOut: Int)
      extends Metadata[ArgMap] {
    def mirror(f: Tx) = this
  }
  object argMapping {
    ...
    def update(arg: Exp[_], id: (Int, Int, Int)): Unit =
      metadata.add(arg, ArgMap(id._1, id._2, id._3))
  ...
  object usersOf {
    def apply(x: Exp[_]): List[Access] =
      metadata[ReadUsers](x).map(_.users).getOrElse(Nil)
    def update(x: Exp[_], users: List[Access]) =
      metadata.add(x, ReadUsers(users))
  }
  ...
  object parFactorOf {
    def apply(x: Exp[_]): Const[Index] =
      metadata[ParFactor](x).map(_.factor).getOrElse(int32(1))
    def update(x: Exp[_], factor: Const[Index]) =
      metadata.add(x, ParFactor(factor))
  }

  /**
    * Parallelization factors which a given node will be unrolled by, prior to unrolling
    */
  case class UnrollFactors(factors: Seq[Seq[Const[Index]]])
      extends Metadata[UnrollFactors] {
    def mirror(f: Tx) = this // Don't mirror constants for now...
  }
  object unrollFactorsOf {
    def apply(x: Exp[_]): Seq[Seq[Const[Index]]] =
      metadata[UnrollFactors](x).map(_.factors).getOrElse(Nil)
    def update(x: Exp[_], factors: Seq[Seq[Const[Index]]]) =
      metadata.add(x, UnrollFactors(factors))
  }

  /**
    * Defines which unrolled duplicate the given symbol is on, for all levels of the control tree above it
    */
  case class UnrollNumbers(nums: Seq[Int]) extends Metadata[UnrollNumbers] {
    def mirror(f: Tx) = this
  }
  object unrollNumsFor {
    def apply(x: Exp[_]): Seq[Int] =
      metadata[UnrollNumbers](x).map(_.nums).getOrElse(Nil)
    def update(x: Exp[_], nums: Seq[Int]) =
      metadata.add(x, UnrollNumbers(nums))
  }

  /**
    * Identifies whether a memory is an accumulator
    */
  case class MAccum(is: Boolean) extends Metadata[MAccum] {
    def mirror(f: Tx) = this
  }
  object isAccum {
  ...
  /**
    * Identifies whether a memory is an accumulator
    */
  case class MInnerAccum(is: Boolean) extends Metadata[MInnerAccum] {
    def mirror(f: Tx) = this
  }
  object isInnerAccum {
  ...
  /**
    ...
    */
  case class DRAMAddress(insts: Long) extends Metadata[DRAMAddress] {
    def mirror(f: Tx) = this
  }
  object dramAddr {
    def apply(e: Exp[_]): Long =
      metadata[DRAMAddress](e).map(_.insts).getOrElse(0)
  ...
  case class MReduceType(func: Option[ReduceFunction])
      extends Metadata[MReduceType] { def mirror(f: Tx) = this }

  object reduceType {
    def apply(e: Exp[_]): Option[ReduceFunction] =
      metadata[MReduceType](e).flatMap(_.func)
    def update(e: Exp[_], func: ReduceFunction) =
      metadata.add(e, MReduceType(Some(func)))
    def update(e: Exp[_], func: Option[ReduceFunction]) =
      metadata.add(e, MReduceType(func))
  }

  case class UnrolledResult(isIt: Boolean) extends Metadata[UnrolledResult] {
    def mirror(f: Tx) = this
  }
  object isReduceResult {
    def apply(e: Exp[_]) = metadata[UnrolledResult](e).exists(_.isIt)
    def update(e: Exp[_], isIt: Boolean) =
      metadata.add(e, UnrolledResult(isIt))
  }

  case class ReduceStarter(isIt: Boolean) extends Metadata[ReduceStarter] {
    def mirror(f: Tx) = this
  }
  ...
  case class PartOfTree(node: Exp[_]) extends Metadata[PartOfTree] {
    def mirror(f: Tx) = this
  }
  object rTreeMap {
  ...
  case class MShouldDuplicate(should: Boolean)
      extends Metadata[MShouldDuplicate] { def mirror(f: Tx) = this }
  object shouldDuplicate {
    def apply(e: Exp[_]) = metadata[MShouldDuplicate](e).exists(_.should)
    def update(e: Exp[_], should: Boolean) =
      metadata.add(e, MShouldDuplicate(should))
  }
