...
object TemplateRunner {
  ...
  def apply(templateMap: Map[String, String => Boolean],
    ...
    val backendName =
      envOrElse("TESTER_BACKENDS", defaultBackend).split(" ").head
        ...
        .map { arg =>
          arg match {
            ...
            case _ => // Figure out tests that match this template and run all
              ...
              templateMap.keys.toSeq.sorted.filter(
                tempRegex.pattern.matcher(_).matches)
          }
        }
        .flatten
        .toArray
    ...
    if (successful > 0) {
      println(
        s"""Templates passing: $successful (${passedTests.mkString(", ")})""")
    }
    if (errors.nonEmpty) {
      println("=" * 80)
