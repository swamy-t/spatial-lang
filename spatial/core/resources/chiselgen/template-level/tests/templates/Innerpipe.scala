...
class InnerpipeTests(c: Innerpipe) extends PeekPokeTester(c) {
  ...
  val maxes = (0 until c.ctrDepth).map { i =>
    math.abs(rnd.nextInt(20)) + 2
  }
  maxes.map { a =>
    println("max of ctr = " + a)
  }
  var cnts = Array.tabulate(c.ctrDepth) { i =>
    0
  }

  (0 until c.ctrDepth).foreach { i =>
    poke(c.io.input.ctr_maxIn(i), maxes(i))
  }
...
class InnerpipeTester extends ChiselFlatSpec {
  ...
  backends foreach { backend =>
    it should s"correctly add randomly generated numbers $backend" in {
      Driver(() => new Innerpipe(2))(c => new InnerpipeTests(c)) should be(
        true)
    }
  }
}
