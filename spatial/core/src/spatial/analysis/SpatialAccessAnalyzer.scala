...
trait SpatialAccessAnalyzer extends AccessPatternAnalyzer {
  ...
  def loopUnapply(x: Exp[_]): Option[Seq[(Seq[Bound[Index]], Seq[Block[_]])]] =
    x match {
      ...
      case Op(e: OpReduce[_]) =>
        Some(List(e.iters -> List(e.map, e.load, e.reduce, e.store)))
      case Op(e: OpMemReduce[_, _]) =>
        Some(
          List(e.itersMap -> List(e.map),
               (e.itersMap ++ e.itersRed) -> List(e.loadAcc,
                                                  e.loadRes,
                                                  e.reduce,
                                                  e.storeAcc)))
      case _ => None
  ...
  def readUnapply(x: Exp[_]): Option[(Exp[_], Seq[Exp[Index]])] = x match {
    case LocalReader(reads) =>
      reads.find(_.addr.isDefined).map { x =>
        (x.mem, x.addr.get)
      }
    case _ => None
  }
  ...
  def writeUnapply(x: Exp[_]): Option[(Exp[_], Seq[Exp[Index]])] = x match {
    case LocalWriter(writes) =>
      writes.find(_.addr.isDefined).map { x =>
        (x.mem, x.addr.get)
      }
    case _ => None
  }
  ...
  override protected def visit(lhs: Sym[_], rhs: Op[_]) = rhs match {
    case e: DenseTransfer[_, _] =>
      accessPatternOf(lhs) = e.iters.map { i =>
        LinearAccess(i)
      }
