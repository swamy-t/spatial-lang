...
trait MatrixApi extends MatrixExp { this: SpatialApi =>
  ...
  implicit class MatrixConstructor(ranges: (Range, Range)) {
    @api
    def apply[A, T](func: (Index, Index) => A)(
        implicit lft: Lift[A, T]): Matrix[T] = {
  ...
  implicit class MatrixOps[T](a: Matrix[T]) {
    ...
    @api def map[R: Type](func: T => R): Matrix[R] =
      matrix(a.data.map(func), a.rows, a.cols)
    @api
    def zip[S: Type, R: Type](b: Matrix[S])(func: (T, S) => R): Matrix[R] =
      matrix(a.data.zip(b.data)(func), a.rows, a.cols)
...
trait MatrixExp { this: SpatialExp =>
  ...
  case class Matrix[T: Type](s: Exp[Matrix[T]]) extends MetaStruct[Matrix[T]] {
    private[spatial] def data(implicit ctx: SrcCtx): MetaArray[T] =
      field[MetaArray[T]]("data")
    ...
    @api def update(i: Index, j: Index, elem: T): Void =
      wrap(array_update(data.s, (i * cols + j).s, elem.s))
  }

  @internal
  def matrix[T: Type](data: MetaArray[T],
                      rows: Index,
                      cols: Index): Matrix[T] = {
  ...
  case class MatrixType[T](child: Meta[T]) extends StructType[Matrix[T]] {
    ...
    override def fields =
      Seq("data" -> ArrayType(child), "rows" -> IntType, "cols" -> IntType)
  }
