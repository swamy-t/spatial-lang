...
trait PIRGenController extends PIRTraversal with PIRCodegen {
  ...
  lazy val allocater = new PIRAllocation {
    val IR: PIRGenController.this.IR.type = PIRGenController.this.IR
  }
  ...
  def generateGlobals() {
    val (mcs, buses) = globals.partition {
      case mc: MemoryController => true; case _ => false
    }
  ...
  override protected def postprocess[S: Type](block: Block[S]): Block[S] = {
    generateFooter()
    msg("Done.")
    val nCUs = cus.values.flatten.filter { cu =>
      cu.allStages.nonEmpty || cu.isDummy
    }.size
  ...
  def emitAllStages(cu: CU) {
    ...
    def emitStages(stages: Iterable[Stage]) = stages.foreach {
      case MapStage(op, inputs, outputs) =>
        ...
        emit(
          s"""Stage(stage($i), operands=List($ins), op=$op, results=List($outs))""")
        i += 1

      case ReduceStage(op, init, in, acc) =>
        emit(s"""val (rs$r, ${quote(acc)}) = Stage.reduce(op=$op, init=${quote(
          init)})""")
        allocatedReduce += acc
        r += 1
    }
  ...
  def preallocateRegisters(cu: CU) = cu.regs.foreach {
    ...
    case reg @ AccumReg(init) =>
      emit(s"val ${quote(reg)} = CU.accum(init = ${quote(init)})")
    case reg: ControlReg if genControlLogic =>
      emit(s"val ${quote(reg)} = CU.ctrl")
    case _ => // No preallocation
  }

  def preallocateFeedbackRegs(cu: CU) = cu.regs.foreach {
    case reg @ FeedbackAddrReg(mem) =>
      emit(s"val ${quote(reg)} = CU.wtAddr(${quote(mem)})")
    case _ => //nothing
  }
  ...
  def emitComponent(x: Any): Unit = x match {
    ...
    case UnitCChain(name) =>
      emit(
        s"""val $name = CounterChain(name = "$name", (Const("0i"), Const("1i"), Const("1i")))""")

    case ctr @ CUCounter(start, end, stride) =>
      emit(s"""val ${ctr.name} = (${quoteInCounter(start)}, ${quoteInCounter(
        end)}, ${quoteInCounter(stride)}) // Counter""")

    case sram: CUMemory =>
      var decl =
        s"""val ${sram.name} = ${quote(sram.mode)}(size = ${sram.size}"""

      sram.writeCtrl match {
        case Some(cchain)                  => decl += s""", writeCtr = ${cchain.name}(0)"""
        case None if sram.mode != SRAMMode => // Ok
        case None =>
          throw new Exception(s"No write controller defined for $sram")
      }
      ...
      if (sram.bufferDepth > 1 && sram.mode != FIFOMode) {
        ...
        sram.swapRead match {
          ...
          case None =>
            throw new Exception(s"No swap read controller defined for $sram")
        }
        ...
        sram.swapWrite match {
          ...
          case None if sram.mode != SRAMMode => // Ok
          case None =>
            throw new Exception(s"No swap write controller defined for $sram")
        }
        decl += s"$buffering)"
      ...
      sram.readAddr match {
        case Some(_: CounterReg | _: ConstReg) =>
          decl += s""".rdAddr(${quote(sram.readAddr.get)})"""
        case Some(_: ReadAddrWire)         =>
        case None if sram.mode == FIFOMode => // ok
        case addr =>
          throw new Exception(
            s"Disallowed memory read address in $sram: $addr")
      }
      sram.writeAddr match {
        case Some(_: CounterReg | _: ConstReg) =>
          decl += s""".wtAddr(${quote(sram.writeAddr.get)})"""
        case Some(_: WriteAddrWire | _: FeedbackAddrReg) =>
        case None if sram.mode != SRAMMode               => // ok
        case addr =>
          throw new Exception(
            s"Disallowed memory write address in $sram: $addr")
      }
    ...
    case mem: OffChip =>
      emit(s"""val ${quote(mem)} = OffChip("${mem.name}")""")
    case bus: InputArg => emit(s"""val ${quote(bus)} = ArgIn("${bus.name}")""")
    case bus: OutputArg =>
      emit(s"""val ${quote(bus)} = ArgOut("${bus.name}")""")
    case bus: ScalarBus =>
      emit(s"""val ${quote(bus)} = Scalar("${bus.name}")""")
    case bus: VectorBus =>
      emit(s"""val ${quote(bus)} = Vector("${bus.name}")""")

    case x =>
      throw new Exception(s"Don't know how to generate PIR component $x")
  }
  ...
  def quote(cu: CU): String = cu.style match {
    case UnitCU if cu.allStages.isEmpty && !cu.isDummy =>
      "Sequential" // outer unit is "Sequential"
    case UnitCU                                          => "UnitPipeline"
    case StreamCU if cu.allStages.isEmpty && !cu.isDummy => "StreamController"
    case StreamCU                                        => "StreamPipeline"
    case UnitStreamCU if cu.allStages.isEmpty && !cu.isDummy =>
  ...
  def quote(reg: LocalComponent): String = reg match {
    ...
    case ValidReg(cchain, idx) =>
      s"${cchain.name}.valids($idx)" // Counter valid
  ...
  def quote(ref: LocalRef): String = ref match {
    ...
    case LocalRef(stage, reg: CounterReg) =>
      if (stage >= 0) s"CU.ctr(stage($stage), ${quote(reg)})" else quote(reg)
    ...
    case LocalRef(stage, reg: FeedbackAddrReg) =>
      s"CU.wtAddr(stage($stage), ${quote(reg)})"
    case LocalRef(stage, reg: FeedbackDataReg) =>
      s"CU.store(stage($stage), ${quote(reg)})"

    case LocalRef(stage, reg: ReduceReg) if allocatedReduce.contains(reg) =>
      quote(reg)
    case LocalRef(stage, reg: ReduceReg) => s"CU.reduce(stage($stage))"
    case LocalRef(stage, reg: AccumReg) =>
      s"CU.accum(stage($stage), ${quote(reg)})"
    case LocalRef(stage, reg: TempReg) =>
      s"CU.temp(stage($stage), ${quote(reg)})"
    case LocalRef(stage, reg: ControlReg) =>
      s"CU.ctrl(stage($stage), ${quote(reg)})"
    case LocalRef(stage, reg: SRAMReadReg) =>
      if (stage >= 0) s"CU.load(stage($stage), ${quote(reg)})"
      else s"${quote(reg)}.load"

    case LocalRef(stage, reg: ScalarIn) =>
      s"CU.scalarIn(stage($stage), ${quote(reg)})"
    case LocalRef(stage, reg: ScalarOut) =>
      s"CU.scalarOut(stage($stage), ${quote(reg)})"
    case LocalRef(stage, reg: VectorIn) =>
      s"CU.vecIn(stage($stage), ${quote(reg)})"
    case LocalRef(stage, reg: VectorOut) =>
      s"CU.vecOut(stage($stage), ${quote(reg)})"
  }
