...
trait RegisterCleanup extends ForwardTransformer {
  ...
  private case object FakeSymbol {
    override def toString = "\"You done goofed\""
  }
  ...
  def delayedMirror[T: Type](lhs: Sym[T], rhs: Op[T], ctrl: Ctrl)(
      implicit ctx: SrcCtx) = () => {
    val key = (lhs, ctrl)
  ...
  override def transform[T: Type](lhs: Sym[T], rhs: Op[T])(
      ...
      if (usersOf(lhs).isEmpty) {
        ...
        val reads = users.map {
          case (parent, uses) =>
            ...
            uses.foreach { use =>
              val subs = (lhs -> read) +: statelessSubstRules
                .getOrElse((use, parent), Nil)
              dbg(s"  $use: $lhs -> $read")
    ...
    case node if isControlNode(lhs) =>
      withCtrl((lhs, false)) { mirrorWithDuplication(lhs, rhs) }
    case _ => mirrorWithDuplication(lhs, rhs)
  }

  private def mirrorWithDuplication[T: Type](lhs: Sym[T], rhs: Op[T])(
    ...
    if (statelessSubstRules.contains((lhs, ctrl))) {
      ...
      val rules = statelessSubstRules((lhs, ctrl)).map {
        case (s, s2) => s -> s2()
      }
    ...
    inlineBlock(
      b, { stms =>
        ...
        if (ctrl != null && statelessSubstRules.contains((ctrl.node, ctrl))) {
          val rules = statelessSubstRules((ctrl.node, ctrl)).map {
            case (s, s2) => s -> s2()
          }
