...
trait ScalaGenDRAM extends ScalaGenMemories {
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    case op @ DRAMNew(dims) =>
      ...
      open(src"val $lhs = {")
      emit(src"""Array.fill(${dims
        .map(quote)
        .mkString("*")} + $elementsPerBurst)(${op.zero})""") //${invalid(op.mA)})""")
      close("}")
    ...
    case e @ FringeDenseLoad(dram, cmdStream, dataStream) =>
      ...
      open(
        src"for (i <- cmd.offset until cmd.offset+cmd.size by $bytesPerWord) {")
      open(src"val data = {")
      oobApply(e.mT, dram, lhs, Nil) {
        emit(src"$dram.apply(i / $bytesPerWord)")
      }
      close("}")
