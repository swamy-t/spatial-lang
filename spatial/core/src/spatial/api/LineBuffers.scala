...
trait LineBufferExp { this: SpatialExp =>
  ...
  case class LineBuffer[T: Meta: Bits](s: Exp[LineBuffer[T]])
      extends Template[LineBuffer[T]] {
    ...
    @api def enq(data: T, en: Bool): Void =
      Void(linebuffer_enq(this.s, data.s, en.s))
  ...
  object LineBuffer {
    @api def apply[T: Meta: Bits](rows: Index, cols: Index): LineBuffer[T] =
      wrap(linebuffer_new[T](rows.s, cols.s))
  }

  /** Type classes **/
  case class LineBufferType[T: Bits](child: Meta[T])
      extends Meta[LineBuffer[T]] {
  ...
  implicit def lineBufferType[T: Meta: Bits]: Meta[LineBuffer[T]] =
    LineBufferType(typ[T])

  class LineBufferIsMemory[T: Meta: Bits] extends Mem[T, LineBuffer] {
    def load(mem: LineBuffer[T], is: Seq[Index], en: Bool)(
        implicit ctx: SrcCtx): T = {
      wrap(linebuffer_load(mem.s, is(0).s, is(1).s, en.s))
    }

    def store(mem: LineBuffer[T], is: Seq[Index], data: T, en: Bool)(
    ...
    def iterators(mem: LineBuffer[T])(implicit ctx: SrcCtx): Seq[Counter] = {
      ...
      stagedDimsOf(mem.s).drop(1).map { d =>
        Counter(0, wrap(d), 1, 1)
      }
    }
  }
  implicit def linebufferIsMemory[T: Meta: Bits]: Mem[T, LineBuffer] =
    new LineBufferIsMemory[T]

  /** IR Nodes **/
  case class LineBufferNew[T: Type: Bits](rows: Exp[Index], cols: Exp[Index])
      extends Op[LineBuffer[T]] {
  ...
  case class LineBufferColSlice[T: Type: Bits](
      ...
      colStart: Exp[Index],
      length: Exp[Index]
  )(implicit val vT: Type[VectorN[T]])
      extends Op[VectorN[T]] {
    def mirror(f: Tx) =
      linebuffer_col_slice(f(linebuffer), f(row), f(colStart), f(length))
  ...
  case class LineBufferRowSlice[T: Type: Bits](
      ...
      extends Op[VectorN[T]] {
    def mirror(f: Tx) =
      linebuffer_row_slice(f(linebuffer), f(rowStart), f(length), f(col))
    override def aliases = Nil
    val mT               = typ[T]
  }
  ...
  /** Constructors **/
  @internal
  def linebuffer_new[T: Type: Bits](rows: Exp[Index], cols: Exp[Index]) = {
    stageMutable(LineBufferNew[T](rows, cols))(ctx)
  }

  @internal
  def linebuffer_col_slice[T: Type: Bits](
      linebuffer: Exp[LineBuffer[T]],
      row: Exp[Index],
      colStart: Exp[Index],
      length: Exp[Index]
    ...
    implicit val vT = length match {
      ...
      case _ =>
        error(
          ctx,
          "Cannot create parameterized or dynamically sized line buffer slice")
        vectorNType[T](0)
    }
  ...
  @internal
  def linebuffer_row_slice[T: Type: Bits](
      linebuffer: Exp[LineBuffer[T]],
      rowStart: Exp[Index],
      length: Exp[Index],
      col: Exp[Index]
  ...
  @internal
  def linebuffer_load[T: Type: Bits](
      linebuffer: Exp[LineBuffer[T]],
      row: Exp[Index],
      col: Exp[Index],
      en: Exp[Bool]
  ...
  @internal
  def linebuffer_enq[T: Type: Bits](
      linebuffer: Exp[LineBuffer[T]],
      data: Exp[T],
      en: Exp[Bool]
  ) = {
