...
/**
  ...
  */
class RegFile(val w: Int,
              val d: Int,
              val numArgIns: Int = 0,
              val numArgOuts: Int = 0)
    extends Module {
  ...
  Predef.assert(numArgIns >= 0,
                s"Invalid numArgIns ($numArgIns): must be >= 0.")
  Predef.assert(numArgOuts >= 0,
                s"Invalid numArgOuts ($numArgOuts): must be >= 0.")
  Predef.assert(
    numArgIns <= d,
    s"numArgIns ($numArgIns) must be less than number of registers ($d)!")
  Predef.assert(
    numArgOuts <= d,
    s"numArgOuts ($numArgOuts) must be less than number of registers ($d)!")

  val regs = List.tabulate(d) { i =>
    val ff = Module(new FF(w))
    ff.io.in := (if (argOutRange contains i)
                   Mux(io.argOuts(regIdx2ArgOut(i)).valid,
                       io.argOuts(regIdx2ArgOut(i)).bits,
                       io.wdata)
                 else io.wdata)
    ff.io.enable := (if (argOutRange contains i)
                       io.argOuts(argOutRange.indexOf(i))
                         .valid | (io.wen & (io.waddr === i.U))
                     else io.wen & (io.waddr === i.U))
    ff.io.init := 0.U
    ff
  }
  ...
  io.argIns := Vec(
    regOuts.zipWithIndex
      .filter { case (arg, idx) => argInRange.contains(idx) }
      .map { _._1 })
}
