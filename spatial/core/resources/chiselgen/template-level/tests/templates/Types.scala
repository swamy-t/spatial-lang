...
class FixedPointTesterTests(c: FixedPointTester) extends PeekPokeTester(c) {
  ...
  num1s.foreach { a =>
    num2s.foreach { b =>
      ...
      step(1)
      val a_rounded = (((a * scala.math.pow(2, c.f)).toInt) / scala.math
        .pow(2, c.f))
      val b_rounded = (((b * scala.math.pow(2, c.f)).toInt) / scala.math
        .pow(2, c.f))
      ...
      expect(c.io.add_result.raw,
             ((a_rounded + b_rounded) * scala.math.pow(2, c.f)).toInt)
      expect(c.io.prod_result.raw,
             (a_rounded * b_rounded * scala.math.pow(2, c.f)).toInt)
      expect(c.io.quotient_result.raw,
             ((a_rounded / b_rounded) * scala.math.pow(2, c.f)).toInt)
      if (a - b >= 0 | c.s)
        expect(c.io.sub_result.raw,
               ((a_rounded - b_rounded) * scala.math.pow(2, c.f)).toInt)
    }
  }
}
