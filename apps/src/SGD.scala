...
object SGD extends SpatialApp { // Regression (Dense) // Args: 40 32 0.0001
  ...
  @virtualize
  def sgd_onept[T: Type: Num](x_in: Array[T],
                              y_in: Array[T],
                              alpha: T,
                              epochs: Int,
                              nn: Int) = {
  ...
  def main() {
    ...
    val ideal_model = Array.tabulate(D) { i =>
      3.to[T]
    }
    val sY = Array.tabulate(N) { i =>
      ideal_model.zip(sX.apply(i)) { _ * _ }.reduce { _ + _ }
    }
    val id = Array.tabulate(D) { i =>
      i
    }
    val ep = Array.tabulate(E) { i =>
      i
    }
...
object SGD_minibatch extends SpatialApp { // Regression (Dense) // Args: 40 32 0.0001
  ...
  @virtualize
  def sgdminibatch[T: Type: Num](x_in: Array[T],
                                 y_in: Array[T],
                                 alpha: T,
                                 epochs: Int,
                                 nn: Int) = {
