...
trait PIRTraversal extends SpatialTraversal {
  ...
  def allocateDRAM(ctrl: Symbol,
                   dram: Symbol,
                   mode: OffchipMemoryMode): MemoryController = {
  ...
  private def allocateReg(reg: Symbol,
                          pipe: Symbol,
                          read: Option[Symbol] = None,
                          write: Option[Symbol] = None): LocalComponent = {
    ...
    if (isLocallyRead && isLocallyWritten && isInnerAccum(reg)) {
      ...
      val init = ConstReg(
        extractConstant(resetValue(reg.asInstanceOf[Exp[Reg[Any]]])))
      AccumReg(init)
      ...
      if (isWrittenByUnitPipe(reg) || isInnerAccum(reg)) {
        ...
        globals += bus
        ScalarIn(bus)
          .asInstanceOf[LocalComponent] // Weird scala type error here
      } else {
        val bus = CUVector(quote(reg))
        globals += bus
        VectorIn(bus)
  ...
  def allocateLocal(x: Symbol,
                    pipe: Symbol,
                    read: Option[Symbol] = None,
                    write: Option[Symbol] = None): LocalComponent = x match {
    ...
    case read @ Def(RegRead(reg)) =>
      allocateLocal(reg, pipe, Some(read), write)
  ...
  def symsOnlyUsedInWriteAddr(stms: Seq[Stm])(results: Seq[Symbol],
                                              exps: Seq[Symbol]) = {
    def mysyms(rhs: Symbol) = rhs match {
      case Def(rhs) =>
        rhs match {
          case LocalWriter(writes) =>
            val addrs = writes.flatMap {
  ...
  def symsOnlyUsedInWriteAddrOrEn(stms: Seq[Stm])(results: Seq[Symbol],
                                                  exps: Seq[Symbol]) = {
    def mysyms(rhs: Any) = rhs match {
      case Def(d) =>
        d match {
          case SRAMStore(sram, dims, is, ofs, data, en) =>
            dyns(sram) ++ dyns(data) //++ dyns(es)
  ...
  def swapBus(cus: Iterable[CU], orig: GlobalBus, swap: GlobalBus) =
    cus.foreach { cu =>
      cu.allStages.foreach { stage =>
        swapBus_stage(stage)
      }
      ...
      def swapBus_stage(stage: Stage): Unit = stage match {
        case stage @ MapStage(op, ins, outs) =>
          stage.ins = ins.map { ref =>
            swapBus_ref(ref)
          }
          stage.outs = outs.map { ref =>
            swapBus_ref(ref)
          }
      ...
      def swapBus_reg(reg: LocalComponent): LocalComponent =
        (reg, swap) match {
          ...
          case (_: LocalPort[_], _) =>
            throw new Exception(s"$swap is not a valid replacement for $orig")
          case _ => reg
        }
      ...
      def swapBus_sram(sram: CUMemory): Unit = {
        ...
        sram.readAddr = sram.readAddr.map { reg =>
          swapBus_readAddr(reg)
        }
        sram.writeAddr = sram.writeAddr.map { reg =>
          swapBus_writeAddr(reg)
        }
        sram.writeStart = sram.writeStart.map { reg =>
          swapBus_localScalar(reg)
        }
        sram.writeEnd = sram.writeEnd.map { reg =>
          swapBus_localScalar(reg)
        }
      }
      def swapBus_cchain(cchain: CUCChain): Unit = cchain match {
        case cc: CChainInstance =>
  ...
  def swapCUs(cus: Iterable[CU], mapping: Map[ACU, ACU]): Unit = cus.foreach {
    cu =>
      cu.cchains.foreach { cchain =>
        swapCU_cchain(cchain)
      }
      cu.parent = cu.parent.map { parent =>
        mapping.getOrElse(parent, parent)
      }
      cu.deps = cu.deps.map { dep =>
        mapping.getOrElse(dep, dep)
      }
      cu.srams.foreach { sram =>
        swapCU_sram(sram)
      }
      ...
      def swapCU_sram(sram: CUMemory) {
        ...
        sram.readAddr.foreach {
          case reg: LocalComponent => swapCU_reg(reg); case _ =>
        }
        sram.writeAddr.foreach {
          case reg: LocalComponent => swapCU_reg(reg); case _ =>
        }
      }
  }
  ...
  abstract class CUContext(val cu: ComputeUnit) {
    ...
    def stageNum: Int =
      stages.count { case stage: MapStage => true; case _ => false } + 1
    def controlStageNum: Int     = controlStages.length
    def prevStage: Option[Stage] = stages.lastOption
    def mapStages: Iterator[MapStage] = stages.iterator.collect {
      case stage: MapStage => stage
    }
    ...
    def reg(x: Symbol): LocalComponent =
      cu.get(x).getOrElse(throw new Exception(s"No register defined for $x"))
    ...
    def ref(reg: LocalComponent,
            out: Boolean,
            stage: Int = stageNum): LocalRef = reg match {
    ...
    def refIn(reg: LocalComponent, stage: Int = stageNum) =
      ref(reg, out = false, stage)
    def refOut(reg: LocalComponent, stage: Int = stageNum) =
      ref(reg, out = true, stage)

    def addOutputFor(e: Symbol)(prev: LocalComponent,
                                out: LocalComponent): Unit =
      addOutput(prev, out, Some(e))
    def addOutput(prev: LocalComponent, out: LocalComponent): Unit =
      addOutput(prev, out, None)
    def addOutput(prev: LocalComponent,
                  out: LocalComponent,
                  e: Option[Symbol]): Unit = {
      mapStages.find { stage =>
        stage.outputMems.contains(prev)
      } match {
        case Some(stage) =>
    ...
    def mem(mem: Symbol, reader: Symbol): CUMemory = {
      memOption(mem, reader).getOrElse(
        throw new Exception(s"Cannot find sram ($mem,$reader) in cu $cu"))
    }
  ...
  case class ComputeContext(override val cu: ComputeUnit)
      extends CUContext(cu) {
  ...
  case class WriteContext(override val cu: ComputeUnit,
                          pipe: Symbol,
                          srams: List[CUMemory])
      extends CUContext(cu) {
  ...
  def propagateReg(exp: Symbol,
                   a: LocalComponent,
                   b: LocalComponent,
                   ctx: CUContext): LocalComponent = (a, b) match {
    ...
    case (a, b) if !isReadable(a) =>
      throw new Exception(s"Cannot propagate for $exp from output-only $a")
    case (a, b) if !isWritable(b) =>
      throw new Exception(s"Cannot propagate for $exp to input-only $b")
