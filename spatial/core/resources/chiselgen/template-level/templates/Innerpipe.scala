...
class Innerpipe(val ctrDepth: Int, val isFSM: Boolean = false) extends Module {
  ...
  if (!isFSM) {
    ...
    val maxFF = List.tabulate(ctrDepth) { i =>
      RegInit(0.U)
    }
    ...
    when(io.input.enable) {
      ...
      when(state === pipeInit.U) {
        ...
        (0 until ctrDepth) foreach { i =>
          maxFF(i) := io.input.ctr_maxIn(i)
        }
        state := pipeReset.U
          ...
          (0 until ctrDepth) foreach { i =>
            io.output.ctr_maxOut(i) := maxFF(i)
          }
        ...
        .elsewhen(state === pipeRun.U) {
          ...
          when(io.input.ctr_done) {
            ...
            (0 until ctrDepth) foreach { i =>
              maxFF(0) := 0.U
            } // TODO: Why do we reset these instead of leaving them?
            state := pipeDone.U
          }.otherwise {
            state := pipeRun.U
          }
...
// Inner pipe
class Streaminner(val ctrDepth: Int, val isFSM: Boolean = false)
    extends Module {
  ...
  io.output.done := Mux(
    ...
    false.B,
    Mux(
      io.input.ctr_done & Mux(io.input.hasStreamIns, true.B, io.input.enable),
      true.B,
      false.B)) // If there is a streamIn for this stage, then we should not require en=true for done to go high
