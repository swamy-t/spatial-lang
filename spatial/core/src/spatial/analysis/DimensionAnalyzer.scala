...
trait DimensionAnalyzer extends SpatialTraversal {
  ...
  private def checkOnchipDims(mem: Exp[_], dims: Seq[Exp[Index]])(
    ...
    dims.zipWithIndex.foreach {
      case (x, i) if x.dependsOnType { case LocalReader(_) => true } =>
        new InvalidOnchipDimensionError(mem, i)
      case (Exact(_), i) =>
  ...
  override protected def visit(lhs: Sym[_], rhs: Op[_]) = rhs match {
    ...
    case _: FIFONew[_] =>
      checkOnchipDims(lhs, List(sizeOf(lhs.asInstanceOf[Exp[FIFO[Any]]])))(
        lhs.ctx)
    case _: LineBufferNew[_] =>
      checkOnchipDims(lhs, stagedDimsOf(lhs))(lhs.ctx)
    case _: RegFileNew[_, _] =>
      checkOnchipDims(lhs, stagedDimsOf(lhs))(lhs.ctx)
  ...
  override protected def postprocess[T: Type](b: Block[T]) = {
    offchips.foreach { dram =>
      val softDims = stagedDimsOf(dram).zipWithIndex.map {
        case (dim, i) =>
          dim match {
            case Op(RegRead(reg))
                if isArgIn(reg) && softValues.contains(reg) =>
              val softDim = softValues(reg)
              assert(softDim.tp match {
                case IntType() => true; case _ => false
              })
              softDim.asInstanceOf[Exp[Index]]
            case _ if isGlobal(dim) => dim.asInstanceOf[Exp[Index]]
            case _ =>
              new InvalidOffchipDimensionError(dram, i)(dram.ctx); int32(0)
          }
      }
