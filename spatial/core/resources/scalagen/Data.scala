...
class FixedPoint(val signed: Boolean, val intBits: Int, val fracBits: Int)
    extends NumberFormat {
  override def equals(obj: Any) = obj match {
    case that: FixedPoint =>
      this.signed == that.signed && this.intBits == that.intBits && this.fracBits == that.fracBits
    case _ => false
  }
}
object FixedPoint {
  ...
  def apply(signed: Boolean, intBits: Int, fracBits: Int) =
    new FixedPoint(signed, intBits, fracBits)
}

class FloatPoint(val sigBits: Int, val expBits: Int) extends NumberFormat {
  override def equals(obj: Any) = obj match {
    case that: FloatPoint =>
      this.sigBits == that.sigBits && this.expBits == that.expBits
    case _ => false
  }
}
object FloatPoint {
...
case class Number(value: BigDecimal, valid: Boolean, fmt: NumberFormat)
    extends Data(valid) {
  def intValue: Int = fmt match {
    case FixedPoint(s, i, 0) => value.toIntExact
    case _ =>
      throw new Exception(
        "Cannot get fixed point representation of a floating point number")
  }
  ...
  def bits: Array[Bit] = fmt match {
    case FixedPoint(s, i, f) =>
      Array.tabulate(i + f) { i =>
        Bit(this.fixValue.testBit(i), this.valid)
      }

    case FloatPoint(g, e) =>
      throw new Exception(
        "TODO: Bitwise operations on floating point valus not yet supported")
  }
  ...
  def +(that: Number) =
    Number(this.value + that.value, this.valid && that.valid, fmt)
  def -(that: Number) =
    Number(this.value - that.value, this.valid && that.valid, fmt)
  def *(that: Number) =
    Number(this.value * that.value, this.valid && that.valid, fmt)
  ...
  def %(that: Number) =
    Number(this.value % that.value, this.valid && that.valid, fmt)
  def &(that: Number) =
    Number(this.fixValue & that.fixValue, this.valid && that.valid, fmt)
  def |(that: Number) =
    Number(this.fixValue | that.fixValue, this.valid && that.valid, fmt)
  def <(that: Number) = Bit(this.value < that.value, this.valid && that.valid)
  def <=(that: Number) =
    Bit(this.value <= that.value, this.valid && that.valid)
  def >(that: Number) = Bit(this.value > that.value, this.valid && that.valid)
  def >=(that: Number) =
    Bit(this.value >= that.value, this.valid && that.valid)
  def !==(that: Number) =
  ...
  def <<(that: Number) =
    Number(this.fixValue << that.intValue, this.valid && that.valid, fmt)
  def >>(that: Number) =
    Number(this.fixValue >> that.intValue, this.valid && that.valid, fmt)
  def >>>(that: Number) = {
  ...
  def until(end: Number) =
    NumberRange(this, end, Number(1), isInclusive = false)
  def to(end: Number) = NumberRange(this, end, Number(1), isInclusive = true)

  override def toString =
    if (valid) { value.toString } else { "X" + value.toString + "X" }
}

// Almost more trouble than it's worth...
case class NumberRange(override val start: Number,
                       override val end: Number,
                       override val step: Number,
                       override val isInclusive: Boolean)
    extends NumericRange[Number](start, end, step, isInclusive)(
      DataImplicits.NumberIsIntegral) {

  override def copy(start: Number, end: Number, step: Number) =
    NumberRange(start, end, step, isInclusive)
...
object Number {
  ...
  def apply(value: Float): Number =
    Number(BigDecimal(value.toDouble), true, FloatFormat)
  def apply(value: Double): Number =
    Number(BigDecimal(value), true, DoubleFormat)
  def apply(value: String, fmt: NumberFormat): Number =
    Number(BigDecimal(value), true, fmt)

  def apply(value: BigInt, valid: Boolean, fmt: NumberFormat): Number =
    fmt match {
      case FixedPoint(s, i, f) =>
        Number(BigDecimal(value) / math.pow(2, f), valid, fmt)
  ...
  def apply(bits: Array[Bit], fmt: NumberFormat): Number = fmt match {
    case FixedPoint(signed, i, f) =>
      ...
      if (signed && bits.last.value) { // Is negative number
        ...
        bits.zipWithIndex.foreach {
          case (bit, i) => if (!bit.value) x = x.flipBit(i)
        }
        Number(x, valid, fmt)
      } else {
    ...
    case FloatPoint(_, _) =>
      throw new Exception(
        "TODO: Bitwise operators not yet defined for floating point")
  }

  def random(max: Number, fmt: NumberFormat): Number = fmt match {
    case FixedPoint(s, i, f) =>
      val bits = Array.tabulate(i + f) { i =>
        Bit(scala.util.Random.nextBoolean())
      }
