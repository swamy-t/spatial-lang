...
object CSV1D extends SpatialApp {
  ...
  def main() {
    ...
    val data = loadCSV1D[T](
      "/home/mattfel/spatial-lang/spatial/core/resources/cppgen/testdata/1d.csv",
      ",")
...
object SSV1D extends SpatialApp {
  ...
  def main() {
    ...
    val tilesize = 16
    val data = loadCSV1D[T](
      "/home/mattfel/spatial-lang/spatial/core/resources/cppgen/testdata/1d.ssv",
      " ")
    val memsize = ArgIn[Int]
    setArg(memsize, data.length.to[Int])
    val srcmem = DRAM[T](memsize)
...
object SSV2D extends SpatialApp {
  ...
  def main() {
    ...
    val data = loadCSV2D[T](
      "/home/mattfel/spatial-lang/spatial/core/resources/cppgen/testdata/2d.ssv",
      " ",
      "\n")
    ...
    printMatrix(data)
    val slice0 = Array.tabulate(memcols) { i =>
      data.apply(0, i)
    }
    val slice1 = Array.tabulate(memcols) { i =>
      data.apply(1, i)
    }
    ...
    Accel {
      ...
      result := Reduce(Reg[T](0.to[T]))(memrows.value by rowtile,
                                        memcols.value by coltile) { (r, c) =>
