...
class LineBuffer(val num_lines: Int,
                 val line_size: Int,
                 val extra_rows_to_buffer: Int,
                 val col_wPar: Int,
                 val col_rPar: Int,
                 val row_wPar: Int,
                 val row_rPar: Int = 1)
  ...
  def this(tuple: (Int, Int, Int, Int, Int, Int, Int)) =
  ...
  // Buffering logic
  val sEn_latch = (0 until num_lines).map { i =>
    Module(new SRFF())
  }
  val sDone_latch = (0 until num_lines).map { i =>
    Module(new SRFF())
  }
  val swap = Wire(Bool())
  ...
  val linebuffer = List.fill(num_lines + extra_rows_to_buffer)(
    Module(
      new SRAM(List(line_size),
               32,
               List(col_wPar max col_rPar),
               List(1),
               List(col_wPar),
               List(col_rPar),
               BankedMemory)))
  ...
  for (i <- 0 until (num_lines + extra_rows_to_buffer)) {
    for (j <- 0 until col_wPar) {
      linebuffer(i).io.w(j).addr(0) := WRITE_countRowPx.io.output
        .count(j) + j.U
  ...
  for (j <- 0 until col_rPar) {
    var linebuf_read_wires_map =
      Array.tabulate(num_lines + extra_rows_to_buffer) { i =>
    ...
    for (i <- 0 until (num_lines)) { // ENHANCEMENT: num_lines -> row par
      io.data_out(i * col_rPar + j) := MuxLookup(
        READ_countRowNum(i).io.output.count,
        0.U,
        linebuf_read_wires_map)
    }
  }
  ...
  val availablePorts = (0 until num_lines).map { p =>
    p
  }
