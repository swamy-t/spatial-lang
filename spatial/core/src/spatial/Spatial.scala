...
protected trait SpatialExp
    ...
    with AlteraVideoExp
    with LineBufferExp
    with RegisterFileExp
    with SwitchExp
    with StateMachineExp
    with EnabledPrimitivesExp
    with NodeClasses
    with NodeUtils
    with ParameterRestrictions
    with SpatialMetadataExp
    with BankingMetadataExp
...
protected trait SpatialApi
    ...
    with AlteraVideoApi
    with LineBufferApi
    with RegisterFileApi
    with SwitchApi
    with StateMachineApi
    with EnabledPrimitivesApi
    with SpatialMetadataApi
    with BankingMetadataApi
    with SpatialImplicits
    with FileIOApi

protected trait ScalaGenSpatial
...
protected trait ChiselGenSpatial
    extends ChiselCodegen
    with ChiselFileGen
    with ChiselGenBool
    with ChiselGenVoid
    with ChiselGenFixPt
    ...
    with ChiselGenSRAM
    with ChiselGenFIFO
    with ChiselGenIfThenElse
    with ChiselGenController
    with ChiselGenMath
    with ChiselGenText
    with ChiselGenDRAM
    with ChiselGenHostTransfer
    with ChiselGenUnrolled
    with ChiselGenVector
    with ChiselGenArray
    with ChiselGenAlteraVideo
    with ChiselGenStream
    with ChiselGenStructs
    with ChiselGenLineBuffer
    with ChiselGenRegFile
    with ChiselGenStateMachine
    with ChiselGenFileIO {

  override val IR: SpatialCompiler
}

protected trait PIRGenSpatial
    extends PIRCodegen
    with PIRFileGen
...
protected trait SpatialCompiler
    extends CompilerCore
    with SpatialApi
    with PIRCommonExp { self =>
...
//lazy val constFolding   = new ConstantFolding { val IR: self.type = self }
  ...
  lazy val unitPipeInsert = new UnitPipeTransformer {
    val IR: self.type = self
  }

  lazy val affineAnalyzer = new SpatialAccessAnalyzer {
    val IR: self.type = self
  }
  lazy val ctrlAnalyzer = new ControlSignalAnalyzer {
    val IR: self.type = self
  }
  ...
  lazy val memAnalyzer = new MemoryAnalyzer {
    val IR: self.type = self; def localMems = ctrlAnalyzer.localMems
  }
  ...
  lazy val controlSanityCheck = new ControllerSanityCheck {
    val IR: self.type = self
  }
  ...
  lazy val transferExpand = new TransferSpecialization {
    val IR: self.type = self
  }

  lazy val reduceAnalyzer = new ReductionAnalyzer { val IR: self.type = self }

  lazy val uctrlAnalyzer = new UnrolledControlAnalyzer {
    val IR: self.type = self
  }
  ...
  lazy val bufferAnalyzer = new BufferAnalyzer {
    val IR: self.type = self; def localMems = uctrlAnalyzer.localMems
  }
  ...
  lazy val argMapper = new ArgMappingAnalyzer {
    val IR: self.type  = self; def memStreams = uctrlAnalyzer.memStreams;
    def argPorts       = uctrlAnalyzer.argPorts;
    def genericStreams = uctrlAnalyzer.genericStreams;
  }

  lazy val scalagen = new ScalaGenSpatial {
    val IR: self.type = self; override def shouldRun = SpatialConfig.enableSim;
    def localMems     = uctrlAnalyzer.localMems
  }
  lazy val chiselgen = new ChiselGenSpatial {
    val IR: self.type      = self;
    override def shouldRun = SpatialConfig.enableSynth
  }
  lazy val pirgen = new PIRGenSpatial {
    val IR: self.type = self; override def shouldRun = SpatialConfig.enablePIR
  }
  lazy val cppgen = new CppGenSpatial {
    val IR: self.type      = self;
    override def shouldRun = SpatialConfig.enableSynth
  }
  lazy val treegen = new TreeGenSpatial {
    val IR: self.type = self; override def shouldRun = SpatialConfig.enableTree
  }
...
trait SpatialApp extends AppCore {
  ...
  private def __target: FPGATarget =
    Targets.targets.find(_.name == SpatialConfig.targetName).getOrElse {
      DefaultTarget
    }
  def target = __target
