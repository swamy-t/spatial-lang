...
trait PIRAllocation extends PIRTraversal {
  ...
  private def controllersHack(pipe: Symbol): List[Symbol] = pipe match {
    case Def(_: ParallelPipe) =>
      childrenOf(pipe).flatMap { child =>
        controllersHack(child)
      }
    case _ => List(pipe)
  }
  // Give top controller or first controller below which is not a Parallel
  private def topControllerHack(access: Access, ctrl: Ctrl): Ctrl =
    ctrl.node match {
  ...
  def pipeDependencies(pipe: Symbol): List[Symbol] = parentOf(pipe) match {
    ...
    case Some(parent) =>
      val childs = childrenOf(parent).map { child =>
        controllersHack(child)
      }
  ...
  def addIterators(cu: PCU,
                   cc: Exp[CounterChain],
                   inds: Seq[Seq[Exp[Index]]],
                   valids: Seq[Seq[Exp[Bool]]]) {
    val cchain = cu.cchains
      .find(_.name == quote(cc))
      .getOrElse(throw new Exception(s"Cannot find counterchain $cc in $cu"))
    inds.zipWithIndex.foreach {
      case (is, i) =>
        is.foreach { index =>
          cu.addReg(index, CounterReg(cchain, i))
        }
    }
    valids.zipWithIndex.foreach {
      case (es, i) =>
        es.foreach { e =>
          cu.addReg(e, ValidReg(cchain, i))
        }
    }
  }
  ...
  def allocateCChains(cu: PCU, pipe: Symbol) {
    ...
    parentHack(pipe).foreach { parent =>
      copyIterators(cu, allocateCU(parent))
    }

    val Def(rhs) = pipe
    val ccs = dyns(rhs).collect {
      case cc @ Def(CounterChainNew(ctrs)) =>
        val counters = ctrs.collect {
          case Def(CounterNew(start, end, stride, _)) =>
            allocateCounter(start, end, stride)
        }
  ...
  def allocateComputeUnit(pipe: Symbol): PCU =
      ...
      pipe, {
        ...
        pipe match {
          case Def(e: UnrolledForeach) =>
            addIterators(cu, e.cchain, e.iters, e.valids)
  ...
  def allocateMemoryUnit(pipe: Symbol): PCU =
    mapping.getOrElseUpdate(
      pipe, {
        val parent = parentHack(pipe).map(allocateCU)
  ...
  def allocateCU(pipe: Symbol): PCU = pipe match {
    ...
    case Def(d) =>
      throw new Exception(s"Don't know how to generate CU for\n  $pipe = $d")
  }

  def prescheduleRegisterRead(reg: Symbol,
                              reader: Symbol,
                              pipe: Option[Symbol]) = {
    dbg(s"Allocating register read: $reader")
    // Register reads may be used by more than one pipe
    readersOf(reg).filter(_.node == reader).map(_.ctrlNode).foreach {
      readCtrl =>
  ...
  def allocateWrittenSRAM(writer: Symbol,
                          mem: Symbol,
                          writerCU: PCU,
                          stages: Seq[PseudoStage]) {
    val srams = readersOf(mem).map { reader =>
      ...
      if (readerCU == writerCU) {
        ...
        val bus =
          if (writerCU.isUnit) CUScalar(quote(mem)) else CUVector(quote(mem))
        globals += bus
  ...
  private def initializeSRAM(sram: CUMemory,
                             mem: Symbol,
                             read: Symbol,
                             cu: PCU) {
    ...
    val writers = writersOf(mem).filter { writer =>
      dispatchOf(writer, mem).contains(instIndex)
    }
    if (writers.length > 1) {
      throw new Exception(
        s"$mem: $writers: PIR currently cannot handle multiple writers")
    }
    val writer = writers.headOption

    val writerCU = writer.map { w =>
      allocateCU(w.ctrlNode)
    }
    val swapWritePipe = writer.flatMap { w =>
      topControllerOf(w, mem, instIndex)
    }
    ...
    val remoteWriteCtrl = writerCU.flatMap { cu =>
      cu.cchains.find {
        case _: UnitCChain | _: CChainInstance => true; case _ => false
      }
    }
    val remoteSwapWriteCtrl = swapWriteCU.flatMap { cu =>
      cu.cchains.find {
        case _: UnitCChain | _: CChainInstance => true; case _ => false
      }
    }
    val remoteSwapReadCtrl = swapReadCU.flatMap { cu =>
      cu.cchains.find {
        case _: UnitCChain | _: CChainInstance => true; case _ => false
      }
    }

    val readCtrl = cu.cchains.find {
      case _: UnitCChain | _: CChainInstance => true; case _ => false
    }
    val writeCtrl = remoteWriteCtrl.flatMap { cc =>
      cu.cchains.find(_.name == cc.name)
    }
    val swapWrite = remoteSwapWriteCtrl.flatMap { cc =>
      cu.cchains.find(_.name == cc.name)
    }
    val swapRead = remoteSwapReadCtrl.flatMap { cc =>
      cu.cchains.find(_.name == cc.name)
    }

    val writeIter = writeCtrl.flatMap { cc =>
      cu.innermostIter(cc)
    }
    val readIter = readCtrl.flatMap { cc =>
      cu.innermostIter(cc)
    }

    val banking =
      if (isFIFO(mem)) Strided(1)
      else {
        val readBanking = bank(mem, read, cu.isUnit)
        val writeBanking = writer
          .map { w =>
            bank(mem, w.node, writerCU.get.isUnit)
          }
          .getOrElse(NoBanks)
...
//      throw new Exception(s"Cannot allocate SRAM for non-buffer symbol $mem")
    ...
    cu.srams
      .find { sram =>
        sram.mem == mem && sram.reader == reader
      }
      .getOrElse {
        ...
        val size = mem match {
          ...
          case m if isFIFO(m) =>
            sizeOf(m.asInstanceOf[Exp[FIFO[Any]]]) match {
  ...
  def prescheduleStages(pipe: Symbol, func: Block[Any]) {
    ...
    var remoteStages
      : Set[Exp[Any]] = Set.empty // Stages to ignore (goes on different CU)
    ...
    def useFifoOnWrite(mem: Exp[Any], value: Exp[Any]): Boolean =
    ...
    def getFifoBounds(
        en: Option[Exp[Any]]): (Option[Exp[Any]], Option[Exp[Any]]) =
      en match {
        case Some(Def(And(Def(FixLeq(start, i1)), Def(FixLt(i2, end))))) =>
          (Some(start), Some(end))
        ...
        case Some(Def(VectorApply(vector, index))) =>
          getFifoBounds(Some(vector))
        case _ => (None, None)
      }

    cu.computeStages
      .clear() // Clear stages so we don't duplicate existing stages
      ...
      case writer @ LocalWriter(writes) if !isControlNode(writer) =>
        ...
        writes.foreach {
          case (mem, value, addr, en) =>
            dbg(
              s"    Checking if $mem write can be implemented as FIFO-on-write:")
            val writeAsFIFO = value.exists { v =>
              useFifoOnWrite(mem, v)
            }
            ...
            if ((isBuffer(mem) || isFIFO(mem)) && writeAsFIFO) {
              ...
              val enableComputation = enable.map { e =>
                getScheduleForAddress(stms)(Seq(e))
              }
              val enableSyms = enableComputation.map { case TP(s, d) => s }

              val (start, end) = getFifoBounds(enable.headOption)

              val startX = start match {
                case start @ Some(Def(_: RegRead[_])) => start; case _ => None
              }
              val endX = end match {
                case end @ Some(Def(_: RegRead[_])) => end; case _ => None
              }

              val remoteWriteStage = FifoOnWriteStage(mem, startX, endX)
              val enStages = startX.map { s =>
                DefStage(s)
              }.toList ++ endX.map { s =>
                DefStage(s)
              }.toList

              allocateWrittenSRAM(writer,
                                  mem,
                                  cu,
                                  enStages ++ List(remoteWriteStage))

              //TODO consider parSRAMStore, localWrite addr will be None
              val indexComputation = addr
                .map { is =>
                  getScheduleForAddress(stms)(is)
                }
                .getOrElse(Nil)
              val indexSyms = indexComputation.map { case TP(s, d) => s }
              remoteStages ++= symsOnlyUsedInWriteAddrOrEn(stms)(
                func.result +: func.effectful,
                indexSyms ++ enableSyms)
              ...
              val indexComputation = addr
                ...
                .getOrElse(Nil)
              val indexSyms = indexComputation.map { case TP(s, d) => s }
              val indexStages = indexSyms.map { s =>
                DefStage(s)
              }
              val flatOpt = addr.map { is =>
                flattenNDIndices(is,
                                 stagedDimsOf(mem.asInstanceOf[Exp[SRAM[_]]]))
              }
              val ad = flatOpt.map(_._1)
              val remoteWriteStage = ad.map { a =>
                WriteAddrStage(mem, a)
              }
              val addrStages = indexStages ++ flatOpt
                .map(_._2)
                .getOrElse(Nil) ++ remoteWriteStage
              ...
              if (indexSyms.nonEmpty && !isLocallyRead) {
                ...
                remoteStages ++= symsOnlyUsedInWriteAddr(stms)(
                  func.result +: func.effectful,
                  indexSyms)
              }
            }
        ...
        reads.foreach {
          case (mem, addr, en) =>
            if (isReg(mem)) {
              ...
              if (!isLocallyWritten || !isLocallyRead || isInnerAccum(mem))
                remoteStages += reader
            } else if (isBuffer(mem)) {
    ...
    val localCompute = stages.filter { s =>
      (isPrimitiveNode(s) || isRegisterRead(s) || isGlobal(s) || isVector(s)) && !remoteStages
        .contains(s)
    }

    // Sanity check
    val trueComputation = localCompute.filterNot {
      case Exact(_) => true; case s => isRegisterRead(s)
    }
    if (isOuterControl(pipe) && trueComputation.nonEmpty) {
      stageWarn(s"Outer control $pipe has compute stages: ")
      trueComputation.foreach {
        case lhs @ Def(rhs) => stageWarn(s"  $lhs = $rhs")
      }
    }

    cu.computeStages ++= localCompute.map { s =>
      DefStage(s, isReduce = reduceType(s).isDefined)
    }
  }

  def prescheduleBurstTransfer(pipe: Symbol,
                               mem: Symbol,
                               ofs: Symbol,
                               len: Symbol,
                               mode: OffchipMemoryMode) = {
    // Ofs and len must either be constants or results of reading registers written in another controller
    val ofsWriter = ofs match {
      case Def(RegRead(reg)) if writersOf(reg).nonEmpty =>
        Some(writersOf(reg).head); case _               => None
    }
    val lenWriter = len match {
      case Def(RegRead(reg)) if writersOf(reg).nonEmpty =>
        Some(writersOf(reg).head); case _               => None
    }

    var ofsCUOpt = ofsWriter.map { writer =>
      allocateCU(writer.ctrlNode)
    }
    var lenCUOpt = lenWriter.map { writer =>
      allocateCU(writer.ctrlNode)
    }
    ...
    mapping.values.foreach { cu =>
      cu.deps = cu.deps.filterNot { dep =>
        dep == ofsCU
      }
    }
  }

  def prescheduleGather(pipe: Symbol,
                        mem: Symbol,
                        local: Exp[SRAM[_]],
                        addrs: Symbol,
                        len: Symbol) {
  ...
  def prescheduleScatter(pipe: Symbol,
                         mem: Symbol,
                         local: Exp[SRAM[_]],
                         addrs: Symbol,
                         len: Symbol) {
  ...
  override protected def visit(lhs: Sym[_], rhs: Op[_]) = rhs match {
    ...
    case _: OpForeach =>
      throw new Exception(s"Disallowed compact op $lhs = $rhs")
    case _: OpReduce[_] =>
      throw new Exception(s"Disallowed compact op $lhs = $rhs")
    ...
    for (cu <- cus) {
      ...
      def addSRAMsReadInWriteStages() {
        ...
        val newWrites = writeStages.foreach {
          case DefStage(reader @ LocalReader(reads), _) =>
            reads foreach {
              case (mem, addr, en) =>
                if (isBuffer(mem) && !cu.srams.exists { sram =>
                      sram.reader == reader && sram.mem == mem
                    }) {
                  val (ownerCU, orig) = owner((reader, mem))
                  val copy            = orig.copyMem(orig.name + "_" + quote(cu.pipe))
                  cu.srams += copy

                  val key = ownerCU.writeStages.keys.find { key =>
                    key.contains(orig)
                  }

                  if (key.isDefined) {
                    cu.writeStages += List(copy) -> ownerCU.writeStages(
                      key.get)
                  }
                  dbg(
                    s"Adding SRAM $copy ($reader, $mem) read in write stage of $cu")
                }
            }
          case _ =>
        }
