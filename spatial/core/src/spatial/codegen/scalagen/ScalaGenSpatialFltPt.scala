...
trait ScalaGenSpatialFltPt extends ScalaGenBits {
  ...
  override protected def quoteConst(c: Const[_]): String = (c.tp, c) match {
    case (FltPtType(g, e), Const(c: BigDecimal)) =>
      s"Number($c, true, FloatPoint($g,$e))"
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case FltConvert(x) =>
      lhs.tp match {
        case FltPtType(g, e) =>
          emit(src"val $lhs = Number($x.value, $x.valid, FloatPoint($g,$e))")
      }
    case FltPtToFixPt(x) =>
      lhs.tp match {
        case FixPtType(s, i, f) =>
          emit(
            src"val $lhs = Number($x.value, $x.valid, FixedPoint($s,$i,$f))")
      }
    case StringToFltPt(x) =>
      lhs.tp match {
        case FltPtType(g, e) =>
          emit(src"val $lhs = Number($x, FloatPoint($g,$e))")
      }

    case FltRandom(Some(max)) =>
      lhs.tp match {
        case FloatType() =>
          emit(src"val $lhs = Number(scala.util.Random.nextFloat() * $max)")
        case DoubleType() =>
          emit(src"val $lhs = Number(scala.util.Random.nextDouble() * $max)")
      }
    case FltRandom(None) =>
      lhs.tp match {
        case FloatType() =>
          emit(src"val $lhs = Number(scala.util.Random.nextFloat())")
        case DoubleType() =>
          emit(src"val $lhs = Number(scala.util.Random.nextDouble()))")
      }
