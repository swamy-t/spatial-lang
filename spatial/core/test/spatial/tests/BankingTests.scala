  ...
  def main() {
    Accel {
      ...
      Foreach(16 by 1) { i =>
        ...
        val sum = Reduce(0)(16 par 2) { j =>
          sram(j)
        } { _ + _ }
        val product = Reduce(0)(16 par 3) { j =>
          sram(j)
        } { _ * _ }
        out1 := sum
        out2 := product
...
class BankingTests extends FlatSpec with Matchers with Exceptions {
  ...
  "TwoDuplicatesSimple" should "have two duplicates of sram" in {
    TwoDuplicatesSimple.main(Array.empty)
  }
  "TwoDuplicatesPachinko" should "have two duplicates of sram" in {
    TwoDuplicatesPachinko.main(Array.empty)
  }

  "LegalFIFOParallelization" should "compile" in {
    LegalFIFOParallelization.main(Array.empty)
  }

  "RegCoalesceTest" should "be coalesced" in {
    RegCoalesceTest.main(Array.empty)
  }
  "SRAMCoalesceTest" should "NOT be coalesced" in {
    SRAMCoalesceTest.main(Array.empty)
  }
  "LinearWriteRandomRead" should "compile" in {
    LinearWriteRandomRead.main(Array.empty)
  }

  a[TestBenchFailed] should be thrownBy {
    IllegalFIFOParallelization.main(Array.empty)
  }
}
